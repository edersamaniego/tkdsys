 ...
 * Page class
 */
class SchoolProgramList extends SchoolProgram
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SchoolProgramList";
    ...
    public $FormName = "fschool_programlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "SchoolProgramAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "SchoolProgramDelete";
        $this->MultiUpdateUrl = "SchoolProgramUpdate";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fschool_programsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->program->AdvancedSearch->toJson(), ","); // Field program
        $filterList = Concat($filterList, $this->timeContractByMonth->AdvancedSearch->toJson(), ","); // Field timeContractByMonth
        ...
        $filterList = Concat($filterList, $this->contractId->AdvancedSearch->toJson(), ","); // Field contractId
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fschool_programsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field timeContractByMonth
        $this->timeContractByMonth->AdvancedSearch->SearchValue = @$filter["x_timeContractByMonth"];
        $this->timeContractByMonth->AdvancedSearch->SearchOperator = @$filter["z_timeContractByMonth"];
        $this->timeContractByMonth->AdvancedSearch->SearchCondition = @$filter["v_timeContractByMonth"];
        $this->timeContractByMonth->AdvancedSearch->SearchValue2 = @$filter["y_timeContractByMonth"];
        $this->timeContractByMonth->AdvancedSearch->SearchOperator2 = @$filter["w_timeContractByMonth"];
        $this->timeContractByMonth->AdvancedSearch->save();
        ...
        // Field contractId
        $this->contractId->AdvancedSearch->SearchValue = @$filter["x_contractId"];
        $this->contractId->AdvancedSearch->SearchOperator = @$filter["z_contractId"];
        $this->contractId->AdvancedSearch->SearchCondition = @$filter["v_contractId"];
        $this->contractId->AdvancedSearch->SearchValue2 = @$filter["y_contractId"];
        $this->contractId->AdvancedSearch->SearchOperator2 = @$filter["w_contractId"];
        $this->contractId->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->timeContractByMonth); // timeContractByMonth
            $this->updateSort($this->value); // value
            $this->updateSort($this->modalityId); // modalityId
            $this->updateSort($this->contractId); // contractId
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->timeContractByMonth->setSort("");
                $this->value->setSort("");
                $this->modalityId->setSort("");
                $this->contractId->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fschool_programlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fschool_programlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("program", $this->createColumnOption("program"));
            $option->add("timeContractByMonth", $this->createColumnOption("timeContractByMonth"));
            $option->add("value", $this->createColumnOption("value"));
            $option->add("modalityId", $this->createColumnOption("modalityId"));
            $option->add("contractId", $this->createColumnOption("contractId"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fschool_programsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fschool_programsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fschool_programlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fschool_programlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fschool_programsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
