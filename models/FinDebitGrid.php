 ...
 * Page class
 */
class FinDebitGrid extends FinDebit
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FinDebitGrid";
    ...
    public $FormName = "ffin_debitgrid";
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_dueDate") &&
            $CurrentForm->hasValue("o_dueDate") &&
            $this->dueDate->CurrentValue != $this->dueDate->DefaultValue &&
            !($this->dueDate->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->dueDate->CurrentValue == $this->dueDate->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_value") &&
            $CurrentForm->hasValue("o_value") &&
            $this->value->CurrentValue != $this->value->DefaultValue &&
            !($this->value->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->value->CurrentValue == $this->value->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_paymentMethod") &&
            $CurrentForm->hasValue("o_paymentMethod") &&
            $this->paymentMethod->CurrentValue != $this->paymentMethod->DefaultValue &&
            !($this->paymentMethod->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->paymentMethod->CurrentValue == $this->paymentMethod->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_checkingAccountId") &&
            $CurrentForm->hasValue("o_checkingAccountId") &&
            $this->checkingAccountId->CurrentValue != $this->checkingAccountId->DefaultValue &&
            !($this->checkingAccountId->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->checkingAccountId->CurrentValue == $this->checkingAccountId->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    public function resetFormError()
    {
        ...
        $this->paymentMethod->clearErrorMessage();
        $this->checkingAccountId->clearErrorMessage();
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_checkingAccountId")) {
            $this->checkingAccountId->setOldValue($CurrentForm->getValue("o_checkingAccountId"));
        }
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "fin_accountspayable") {
            $this->accountId->CurrentValue = $this->accountId->getSessionValue();
        }
        if ($this->getCurrentMasterTable() == "fin_checkingaccount") {
            $this->accountId->CurrentValue = $this->accountId->getSessionValue();
        }
        ...
        // checkingAccountId
        $this->checkingAccountId->setDbValueDef($rsnew, $this->checkingAccountId->CurrentValue, null, false);

        // accountId
        if ($this->accountId->getSessionValue() != "") {
            $rsnew['accountId'] = $this->accountId->getSessionValue();
        }
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            $detailKeys = [];
            $detailKeys["accountId"] = $this->accountId->CurrentValue;
            $masterTable = Container("fin_accountspayable");
            ...
            if (!EmptyValue($masterFilter)) {
                ...
                } elseif ($this->getCurrentMasterTable() == "fin_accountspayable") {
            ...
            if (!EmptyValue($masterFilter)) {
                ...
                if ($rsmaster = $masterTable->loadRs($masterFilter)->fetchAssociative()) {
                    $validMasterKey = $Security->isValidUserID($rsmaster['schoolId']);
                } elseif ($this->getCurrentMasterTable() == "fin_checkingaccount") {
                    $validMasterKey = false;
                }
                if (!$validMasterKey) {
                    $masterUserIdMsg = str_replace("%c", CurrentUserID(), $Language->phrase("UnAuthorizedMasterUserID"));
