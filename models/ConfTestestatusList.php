 ...
 * Page class
 */
class ConfTestestatusList extends ConfTestestatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ConfTestestatusList";
    ...
    public $FormName = "fconf_testestatuslist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ConfTestestatusAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ConfTestestatusDelete";
        $this->MultiUpdateUrl = "ConfTestestatusUpdate";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fconf_testestatussrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->testStatus->AdvancedSearch->toJson(), ","); // Field testStatus
        $filterList = Concat($filterList, $this->statusEN->AdvancedSearch->toJson(), ","); // Field statusEN
        $filterList = Concat($filterList, $this->statusES->AdvancedSearch->toJson(), ","); // Field statusES
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fconf_testestatussrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field testStatus
        $this->testStatus->AdvancedSearch->SearchValue = @$filter["x_testStatus"];
        $this->testStatus->AdvancedSearch->SearchOperator = @$filter["z_testStatus"];
        $this->testStatus->AdvancedSearch->SearchCondition = @$filter["v_testStatus"];
        $this->testStatus->AdvancedSearch->SearchValue2 = @$filter["y_testStatus"];
        $this->testStatus->AdvancedSearch->SearchOperator2 = @$filter["w_testStatus"];
        $this->testStatus->AdvancedSearch->save();

        // Field statusEN
        $this->statusEN->AdvancedSearch->SearchValue = @$filter["x_statusEN"];
        $this->statusEN->AdvancedSearch->SearchOperator = @$filter["z_statusEN"];
        $this->statusEN->AdvancedSearch->SearchCondition = @$filter["v_statusEN"];
        $this->statusEN->AdvancedSearch->SearchValue2 = @$filter["y_statusEN"];
        $this->statusEN->AdvancedSearch->SearchOperator2 = @$filter["w_statusEN"];
        $this->statusEN->AdvancedSearch->save();

        // Field statusES
        $this->statusES->AdvancedSearch->SearchValue = @$filter["x_statusES"];
        $this->statusES->AdvancedSearch->SearchOperator = @$filter["z_statusES"];
        $this->statusES->AdvancedSearch->SearchCondition = @$filter["v_statusES"];
        $this->statusES->AdvancedSearch->SearchValue2 = @$filter["y_statusES"];
        $this->statusES->AdvancedSearch->SearchOperator2 = @$filter["w_statusES"];
        $this->statusES->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->testStatus;
        $searchFlds[] = &$this->statusEN;
        $searchFlds[] = &$this->statusES;
        $searchFlds[] = &$this->obs;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->testStatus); // testStatus
            $this->updateSort($this->statusEN); // statusEN
            $this->updateSort($this->statusES); // statusES
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id->setSort("");
                $this->testStatus->setSort("");
                $this->statusEN->setSort("");
                $this->statusES->setSort("");
                $this->obs->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fconf_testestatuslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fconf_testestatuslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("testStatus", $this->createColumnOption("testStatus"));
            $option->add("statusEN", $this->createColumnOption("statusEN"));
            $option->add("statusES", $this->createColumnOption("statusES"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fconf_testestatussrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fconf_testestatussrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fconf_testestatuslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fconf_testestatuslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fconf_testestatussrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
