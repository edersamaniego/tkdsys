 ...
 * Page class
 */
class FedRankGrid extends FedRank
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedRankGrid";
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_rankBR") &&
            $CurrentForm->hasValue("o_rankBR") &&
            $this->rankBR->CurrentValue != $this->rankBR->DefaultValue &&
            !($this->rankBR->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->rankBR->CurrentValue == $this->rankBR->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_rankUS") &&
            $CurrentForm->hasValue("o_rankUS") &&
            $this->rankUS->CurrentValue != $this->rankUS->DefaultValue &&
            !($this->rankUS->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->rankUS->CurrentValue == $this->rankUS->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_rankES") &&
            $CurrentForm->hasValue("o_rankES") &&
            $this->rankES->CurrentValue != $this->rankES->DefaultValue &&
            !($this->rankES->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->rankES->CurrentValue == $this->rankES->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_ranking") &&
            $CurrentForm->hasValue("o_ranking") &&
            $this->ranking->CurrentValue != $this->ranking->DefaultValue &&
            !($this->ranking->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->ranking->CurrentValue == $this->ranking->getSessionValue())
        ) {
            return false;
        }
        if (
    ...
    public function resetFormError()
    {
        ...
        $this->rankBR->clearErrorMessage();
        $this->rankUS->clearErrorMessage();
        $this->rankES->clearErrorMessage();
        $this->ranking->clearErrorMessage();
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'rankBR' first before field var 'x_rankBR'
        $val = $CurrentForm->hasValue("rankBR") ? $CurrentForm->getValue("rankBR") : $CurrentForm->getValue("x_rankBR");
        if (!$this->rankBR->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->rankBR->Visible = false; // Disable update for API request
            } else {
                $this->rankBR->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_rankBR")) {
            $this->rankBR->setOldValue($CurrentForm->getValue("o_rankBR"));
        }

        // Check field name 'rankUS' first before field var 'x_rankUS'
        $val = $CurrentForm->hasValue("rankUS") ? $CurrentForm->getValue("rankUS") : $CurrentForm->getValue("x_rankUS");
        if (!$this->rankUS->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->rankUS->Visible = false; // Disable update for API request
            } else {
                $this->rankUS->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_rankUS")) {
            $this->rankUS->setOldValue($CurrentForm->getValue("o_rankUS"));
        }

        // Check field name 'rankES' first before field var 'x_rankES'
        $val = $CurrentForm->hasValue("rankES") ? $CurrentForm->getValue("rankES") : $CurrentForm->getValue("x_rankES");
        if (!$this->rankES->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->rankES->Visible = false; // Disable update for API request
            } else {
                $this->rankES->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_rankES")) {
            $this->rankES->setOldValue($CurrentForm->getValue("o_rankES"));
        }

        // Check field name 'ranking' first before field var 'x_ranking'
        $val = $CurrentForm->hasValue("ranking") ? $CurrentForm->getValue("ranking") : $CurrentForm->getValue("x_ranking");
        if (!$this->ranking->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ranking->Visible = false; // Disable update for API request
            } else {
                $this->ranking->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_ranking")) {
            $this->ranking->setOldValue($CurrentForm->getValue("o_ranking"));
        }
        ...
        if (!$this->nextrankId->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->nextrankId->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->rankBR->CurrentValue = $this->rankBR->FormValue;
        $this->rankUS->CurrentValue = $this->rankUS->FormValue;
        $this->rankES->CurrentValue = $this->rankES->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->rankBR->Raw) {
                $this->rankBR->CurrentValue = HtmlDecode($this->rankBR->CurrentValue);
            }
            $this->rankBR->EditValue = HtmlEncode($this->rankBR->CurrentValue);
            ...
            if (!$this->rankUS->Raw) {
                $this->rankUS->CurrentValue = HtmlDecode($this->rankUS->CurrentValue);
            }
            $this->rankUS->EditValue = HtmlEncode($this->rankUS->CurrentValue);
            ...
            if (!$this->rankES->Raw) {
                $this->rankES->CurrentValue = HtmlDecode($this->rankES->CurrentValue);
            }
            $this->rankES->EditValue = HtmlEncode($this->rankES->CurrentValue);
            ...
            if (!$this->ranking->Raw) {
                $this->ranking->CurrentValue = HtmlDecode($this->ranking->CurrentValue);
            }
            $this->ranking->EditValue = HtmlEncode($this->ranking->CurrentValue);
            ...
            $this->nextrankId->EditValue = HtmlEncode($this->nextrankId->CurrentValue);
            $curVal = strval($this->nextrankId->CurrentValue);
            if ($curVal != "") {
                ...
                if ($this->nextrankId->EditValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->nextrankId->EditValue = HtmlEncode(FormatNumber($this->nextrankId->CurrentValue, $this->nextrankId->formatPattern()));
                    }
                }
            } else {
    ...
    protected function validateForm()
    {
        ...
        if ($this->rankBR->Required) {
            if (!$this->rankBR->IsDetailKey && EmptyValue($this->rankBR->FormValue)) {
                $this->rankBR->addErrorMessage(str_replace("%s", $this->rankBR->caption(), $this->rankBR->RequiredErrorMessage));
            }
        }
        if ($this->rankUS->Required) {
            if (!$this->rankUS->IsDetailKey && EmptyValue($this->rankUS->FormValue)) {
                $this->rankUS->addErrorMessage(str_replace("%s", $this->rankUS->caption(), $this->rankUS->RequiredErrorMessage));
            }
        }
        if ($this->rankES->Required) {
            if (!$this->rankES->IsDetailKey && EmptyValue($this->rankES->FormValue)) {
                $this->rankES->addErrorMessage(str_replace("%s", $this->rankES->caption(), $this->rankES->RequiredErrorMessage));
            }
        }
        if ($this->ranking->Required) {
            if (!$this->ranking->IsDetailKey && EmptyValue($this->ranking->FormValue)) {
                $this->ranking->addErrorMessage(str_replace("%s", $this->ranking->caption(), $this->ranking->RequiredErrorMessage));
            }
        }
        ...
        if (!CheckInteger($this->nextrankId->FormValue)) {
    ...
    protected function editRow()
    {
        ...
        // rankBR
        $this->rankBR->setDbValueDef($rsnew, $this->rankBR->CurrentValue, null, $this->rankBR->ReadOnly);

        // rankUS
        $this->rankUS->setDbValueDef($rsnew, $this->rankUS->CurrentValue, null, $this->rankUS->ReadOnly);

        // rankES
        $this->rankES->setDbValueDef($rsnew, $this->rankES->CurrentValue, null, $this->rankES->ReadOnly);

        // ranking
        $this->ranking->setDbValueDef($rsnew, $this->ranking->CurrentValue, null, $this->ranking->ReadOnly);
        ...
        // Check referential integrity for master table 'fed_martialarts'
        $detailKeys = [];
        $keyValue = $rsnew['martialArtsId'] ?? $rsold['martialArtsId'];
        $detailKeys['martialArtsId'] = $keyValue;
        $masterTable = Container("fed_martialarts");
        ...
        if (!$validMasterRecord) {
            $relatedRecordMsg = str_replace("%t", "fed_martialarts", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "fed_martialarts") {
            $this->martialArtsId->CurrentValue = $this->martialArtsId->getSessionValue();
        }
        ...
        // rankBR
        $this->rankBR->setDbValueDef($rsnew, $this->rankBR->CurrentValue, null, false);

        // rankUS
        $this->rankUS->setDbValueDef($rsnew, $this->rankUS->CurrentValue, null, false);

        // rankES
        $this->rankES->setDbValueDef($rsnew, $this->rankES->CurrentValue, null, false);

        // ranking
        $this->ranking->setDbValueDef($rsnew, $this->ranking->CurrentValue, null, false);

        // nextrankId
        $this->nextrankId->setDbValueDef($rsnew, $this->nextrankId->CurrentValue, null, false);

        // martialArtsId
        if ($this->martialArtsId->getSessionValue() != "") {
            $rsnew['martialArtsId'] = $this->martialArtsId->getSessionValue();
        }

        // Update current values
        $this->setCurrentValues($rsnew);

        // Check referential integrity for master table 'fed_rank'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["martialArtsId"] = $this->martialArtsId->getSessionValue();
        $masterTable = Container("fed_martialarts");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
