 ...
 * Page class
 */
class FedLicenseschoolList extends FedLicenseschool
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedLicenseschoolList";
    ...
    public $FormName = "ffed_licenseschoollist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "FedLicenseschoolAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FedLicenseschoolDelete";
        $this->MultiUpdateUrl = "FedLicenseschoolUpdate";
    ...
    public function run()
    {
        ...
        $this->installment->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffed_licenseschoolsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->application->AdvancedSearch->toJson(), ","); // Field application
        $filterList = Concat($filterList, $this->dateLicense->AdvancedSearch->toJson(), ","); // Field dateLicense
        $filterList = Concat($filterList, $this->dateStart->AdvancedSearch->toJson(), ","); // Field dateStart
        $filterList = Concat($filterList, $this->dateFinish->AdvancedSearch->toJson(), ","); // Field dateFinish
        $filterList = Concat($filterList, $this->schooltype->AdvancedSearch->toJson(), ","); // Field schooltype
        ...
        $filterList = Concat($filterList, $this->masterSchool->AdvancedSearch->toJson(), ","); // Field masterSchool
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffed_licenseschoolsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field dateLicense
        $this->dateLicense->AdvancedSearch->SearchValue = @$filter["x_dateLicense"];
        $this->dateLicense->AdvancedSearch->SearchOperator = @$filter["z_dateLicense"];
        $this->dateLicense->AdvancedSearch->SearchCondition = @$filter["v_dateLicense"];
        $this->dateLicense->AdvancedSearch->SearchValue2 = @$filter["y_dateLicense"];
        $this->dateLicense->AdvancedSearch->SearchOperator2 = @$filter["w_dateLicense"];
        $this->dateLicense->AdvancedSearch->save();

        // Field dateStart
        $this->dateStart->AdvancedSearch->SearchValue = @$filter["x_dateStart"];
        $this->dateStart->AdvancedSearch->SearchOperator = @$filter["z_dateStart"];
        $this->dateStart->AdvancedSearch->SearchCondition = @$filter["v_dateStart"];
        $this->dateStart->AdvancedSearch->SearchValue2 = @$filter["y_dateStart"];
        $this->dateStart->AdvancedSearch->SearchOperator2 = @$filter["w_dateStart"];
        $this->dateStart->AdvancedSearch->save();

        // Field dateFinish
        $this->dateFinish->AdvancedSearch->SearchValue = @$filter["x_dateFinish"];
        $this->dateFinish->AdvancedSearch->SearchOperator = @$filter["z_dateFinish"];
        $this->dateFinish->AdvancedSearch->SearchCondition = @$filter["v_dateFinish"];
        $this->dateFinish->AdvancedSearch->SearchValue2 = @$filter["y_dateFinish"];
        $this->dateFinish->AdvancedSearch->SearchOperator2 = @$filter["w_dateFinish"];
        $this->dateFinish->AdvancedSearch->save();

        // Field schooltype
        $this->schooltype->AdvancedSearch->SearchValue = @$filter["x_schooltype"];
        $this->schooltype->AdvancedSearch->SearchOperator = @$filter["z_schooltype"];
        $this->schooltype->AdvancedSearch->SearchCondition = @$filter["v_schooltype"];
        $this->schooltype->AdvancedSearch->SearchValue2 = @$filter["y_schooltype"];
        $this->schooltype->AdvancedSearch->SearchOperator2 = @$filter["w_schooltype"];
        $this->schooltype->AdvancedSearch->save();
        ...
        // Field masterSchool
        $this->masterSchool->AdvancedSearch->SearchValue = @$filter["x_masterSchool"];
        $this->masterSchool->AdvancedSearch->SearchOperator = @$filter["z_masterSchool"];
        $this->masterSchool->AdvancedSearch->SearchCondition = @$filter["v_masterSchool"];
        $this->masterSchool->AdvancedSearch->SearchValue2 = @$filter["y_masterSchool"];
        $this->masterSchool->AdvancedSearch->SearchOperator2 = @$filter["w_masterSchool"];
        $this->masterSchool->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->dateLicense, $default, false); // dateLicense
        $this->buildSearchSql($where, $this->dateStart, $default, false); // dateStart
        $this->buildSearchSql($where, $this->dateFinish, $default, false); // dateFinish
        $this->buildSearchSql($where, $this->schooltype, $default, false); // schooltype
        $this->buildSearchSql($where, $this->value, $default, false); // value
        $this->buildSearchSql($where, $this->installment, $default, false); // installment
        $this->buildSearchSql($where, $this->masterSchool, $default, false); // masterSchool
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->dateLicense->AdvancedSearch->save(); // dateLicense
            $this->dateStart->AdvancedSearch->save(); // dateStart
            $this->dateFinish->AdvancedSearch->save(); // dateFinish
            $this->schooltype->AdvancedSearch->save(); // schooltype
            $this->value->AdvancedSearch->save(); // value
            $this->installment->AdvancedSearch->save(); // installment
            $this->masterSchool->AdvancedSearch->save(); // masterSchool
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->dateLicense->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->dateStart->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->dateFinish->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->schooltype->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->value->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->installment->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->masterSchool->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->dateLicense->AdvancedSearch->unsetSession();
        $this->dateStart->AdvancedSearch->unsetSession();
        $this->dateFinish->AdvancedSearch->unsetSession();
        $this->schooltype->AdvancedSearch->unsetSession();
        $this->value->AdvancedSearch->unsetSession();
        $this->installment->AdvancedSearch->unsetSession();
        $this->masterSchool->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->dateLicense->AdvancedSearch->load();
        $this->dateStart->AdvancedSearch->load();
        $this->dateFinish->AdvancedSearch->load();
        $this->schooltype->AdvancedSearch->load();
        $this->value->AdvancedSearch->load();
        $this->installment->AdvancedSearch->load();
        $this->masterSchool->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->dateLicense); // dateLicense
            $this->updateSort($this->dateStart); // dateStart
            $this->updateSort($this->dateFinish); // dateFinish
            $this->updateSort($this->schooltype); // schooltype
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->dateLicense->setSort("");
                $this->dateStart->setSort("");
                $this->dateFinish->setSort("");
                $this->schooltype->setSort("");
                $this->value->setSort("");
                $this->installment->setSort("");
                $this->masterSchool->setSort("");
                $this->school->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_licenseschoollist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_licenseschoollist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("application", $this->createColumnOption("application"));
            $option->add("dateLicense", $this->createColumnOption("dateLicense"));
            $option->add("dateStart", $this->createColumnOption("dateStart"));
            $option->add("dateFinish", $this->createColumnOption("dateFinish"));
            $option->add("schooltype", $this->createColumnOption("schooltype"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffed_licenseschoolsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffed_licenseschoolsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffed_licenseschoollist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // application
        if ($this->application->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // dateLicense
        if ($this->dateLicense->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->dateLicense->AdvancedSearch->SearchValue != "" || $this->dateLicense->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // dateStart
        if ($this->dateStart->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->dateStart->AdvancedSearch->SearchValue != "" || $this->dateStart->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // dateFinish
        if ($this->dateFinish->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->dateFinish->AdvancedSearch->SearchValue != "" || $this->dateFinish->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // schooltype
        if ($this->schooltype->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->schooltype->AdvancedSearch->SearchValue != "" || $this->schooltype->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // installment
        if ($this->installment->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->installment->AdvancedSearch->SearchValue != "" || $this->installment->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // masterSchool
        if ($this->masterSchool->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->masterSchool->AdvancedSearch->SearchValue != "" || $this->masterSchool->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->application->EditValue = HtmlEncode($this->application->AdvancedSearch->SearchValue);
            ...
            $this->dateLicense->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->dateLicense->AdvancedSearch->SearchValue, $this->dateLicense->formatPattern()), $this->dateLicense->formatPattern()));
            $this->dateLicense->PlaceHolder = RemoveHtml($this->dateLicense->caption());
            $this->dateLicense->setupEditAttributes();
            $this->dateLicense->EditCustomAttributes = "";
            $this->dateLicense->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->dateLicense->AdvancedSearch->SearchValue2, $this->dateLicense->formatPattern()), $this->dateLicense->formatPattern()));
            $this->dateLicense->PlaceHolder = RemoveHtml($this->dateLicense->caption());

            // dateStart
            $this->dateStart->setupEditAttributes();
            $this->dateStart->EditCustomAttributes = "";
            $this->dateStart->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->dateStart->AdvancedSearch->SearchValue, $this->dateStart->formatPattern()), $this->dateStart->formatPattern()));
            $this->dateStart->PlaceHolder = RemoveHtml($this->dateStart->caption());
            $this->dateStart->setupEditAttributes();
            $this->dateStart->EditCustomAttributes = "";
            $this->dateStart->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->dateStart->AdvancedSearch->SearchValue2, $this->dateStart->formatPattern()), $this->dateStart->formatPattern()));
            $this->dateStart->PlaceHolder = RemoveHtml($this->dateStart->caption());

            // dateFinish
            $this->dateFinish->setupEditAttributes();
            $this->dateFinish->EditCustomAttributes = "";
            $this->dateFinish->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->dateFinish->AdvancedSearch->SearchValue, $this->dateFinish->formatPattern()), $this->dateFinish->formatPattern()));
            $this->dateFinish->PlaceHolder = RemoveHtml($this->dateFinish->caption());
            $this->dateFinish->setupEditAttributes();
            $this->dateFinish->EditCustomAttributes = "";
            $this->dateFinish->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->dateFinish->AdvancedSearch->SearchValue2, $this->dateFinish->formatPattern()), $this->dateFinish->formatPattern()));
            $this->dateFinish->PlaceHolder = RemoveHtml($this->dateFinish->caption());
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckDate($this->dateLicense->AdvancedSearch->SearchValue, $this->dateLicense->formatPattern())) {
            $this->dateLicense->addErrorMessage($this->dateLicense->getErrorMessage(false));
        }
        if (!CheckDate($this->dateLicense->AdvancedSearch->SearchValue2, $this->dateLicense->formatPattern())) {
            $this->dateLicense->addErrorMessage($this->dateLicense->getErrorMessage(false));
        }
        if (!CheckDate($this->dateStart->AdvancedSearch->SearchValue, $this->dateStart->formatPattern())) {
            $this->dateStart->addErrorMessage($this->dateStart->getErrorMessage(false));
        }
        if (!CheckDate($this->dateStart->AdvancedSearch->SearchValue2, $this->dateStart->formatPattern())) {
            $this->dateStart->addErrorMessage($this->dateStart->getErrorMessage(false));
        }
        if (!CheckDate($this->dateFinish->AdvancedSearch->SearchValue, $this->dateFinish->formatPattern())) {
            $this->dateFinish->addErrorMessage($this->dateFinish->getErrorMessage(false));
        }
        if (!CheckDate($this->dateFinish->AdvancedSearch->SearchValue2, $this->dateFinish->formatPattern())) {
            $this->dateFinish->addErrorMessage($this->dateFinish->getErrorMessage(false));
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffed_licenseschoollist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffed_licenseschoolsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->dateLicense->Visible || $this->dateStart->Visible || $this->dateFinish->Visible;
    }
