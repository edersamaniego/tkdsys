 ...
 * Page class
 */
class FinCostcenterList extends FinCostcenter
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'fin_costcenter';

    // Page object name
    public $PageObjName = "FinCostcenterList";
    ...
    public $FormName = "ffin_costcenterlist";
    ...
    public function __construct()
    {
        ...
        // Table object (fin_costcenter)
        if (!isset($GLOBALS["fin_costcenter"]) || get_class($GLOBALS["fin_costcenter"]) == PROJECT_NAMESPACE . "fin_costcenter") {
            $GLOBALS["fin_costcenter"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "FinCostcenterAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FinCostcenterDelete";
        $this->MultiUpdateUrl = "FinCostcenterUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'fin_costcenter');
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffin_costcentersrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->costCenter->AdvancedSearch->toJson(), ","); // Field costCenter
        ...
        $filterList = Concat($filterList, $this->organization->AdvancedSearch->toJson(), ","); // Field organization
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffin_costcentersrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field costCenter
        $this->costCenter->AdvancedSearch->SearchValue = @$filter["x_costCenter"];
        $this->costCenter->AdvancedSearch->SearchOperator = @$filter["z_costCenter"];
        $this->costCenter->AdvancedSearch->SearchCondition = @$filter["v_costCenter"];
        $this->costCenter->AdvancedSearch->SearchValue2 = @$filter["y_costCenter"];
        $this->costCenter->AdvancedSearch->SearchOperator2 = @$filter["w_costCenter"];
        $this->costCenter->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->costCenter); // costCenter
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffin_costcenterlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffin_costcenterlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("costCenter", $this->createColumnOption("costCenter"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffin_costcentersrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffin_costcentersrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffin_costcenterlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->costCenter->setDbValue($row['costCenter']);
    ...
    protected function newRow()
    {
        ...
        $row['costCenter'] = $this->costCenter->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // costCenter
            $this->costCenter->ViewValue = $this->costCenter->CurrentValue;
            $this->costCenter->ViewCustomAttributes = "";
            ...
            // costCenter
            $this->costCenter->LinkCustomAttributes = "";
            $this->costCenter->HrefValue = "";
            $this->costCenter->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffin_costcenterlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffin_costcentersrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
