 ...
 * Page class
 */
class FedFilesList extends FedFiles
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedFilesList";
    ...
    public $FormName = "ffed_fileslist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "FedFilesAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FedFilesDelete";
        $this->MultiUpdateUrl = "FedFilesUpdate";
    ...
    public $MultiColumnGridClass = "row-cols-md-3";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffed_filessrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->name->AdvancedSearch->toJson(), ","); // Field name
        $filterList = Concat($filterList, $this->thumbs->AdvancedSearch->toJson(), ","); // Field thumbs
        ...
        $filterList = Concat($filterList, $this->dateUpdate->AdvancedSearch->toJson(), ","); // Field dateUpdate
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffed_filessrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field thumbs
        $this->thumbs->AdvancedSearch->SearchValue = @$filter["x_thumbs"];
        $this->thumbs->AdvancedSearch->SearchOperator = @$filter["z_thumbs"];
        $this->thumbs->AdvancedSearch->SearchCondition = @$filter["v_thumbs"];
        $this->thumbs->AdvancedSearch->SearchValue2 = @$filter["y_thumbs"];
        $this->thumbs->AdvancedSearch->SearchOperator2 = @$filter["w_thumbs"];
        $this->thumbs->AdvancedSearch->save();
        ...
        // Field dateUpdate
        $this->dateUpdate->AdvancedSearch->SearchValue = @$filter["x_dateUpdate"];
        $this->dateUpdate->AdvancedSearch->SearchOperator = @$filter["z_dateUpdate"];
        $this->dateUpdate->AdvancedSearch->SearchCondition = @$filter["v_dateUpdate"];
        $this->dateUpdate->AdvancedSearch->SearchValue2 = @$filter["y_dateUpdate"];
        $this->dateUpdate->AdvancedSearch->SearchOperator2 = @$filter["w_dateUpdate"];
        $this->dateUpdate->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->thumbs, $default, false); // thumbs
        ...
        $this->buildSearchSql($where, $this->dateUpdate, $default, false); // dateUpdate
        $this->buildSearchSql($where, $this->dateCreation, $default, false); // dateCreation
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->thumbs->AdvancedSearch->save(); // thumbs
            ...
            $this->dateUpdate->AdvancedSearch->save(); // dateUpdate
            $this->dateCreation->AdvancedSearch->save(); // dateCreation
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->thumbs->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->dateUpdate->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->dateCreation->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->thumbs->AdvancedSearch->unsetSession();
        ...
        $this->dateUpdate->AdvancedSearch->unsetSession();
        $this->dateCreation->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->thumbs->AdvancedSearch->load();
        $this->description->AdvancedSearch->load();
        $this->link->AdvancedSearch->load();
        $this->status->AdvancedSearch->load();
        $this->category->AdvancedSearch->load();
        $this->type->AdvancedSearch->load();
        $this->dateUpdate->AdvancedSearch->load();
        $this->dateCreation->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->thumbs); // thumbs
    ...
    protected function setupListOptions()
    {
        ...
        $this->MultiColumnLayout = Get(Config("PAGE_LAYOUT"));
        ...
        if ($this->MultiColumnLayout == "cards") { // Multi column layout
            ...
            $this->ListOptions->TagClassName = "ew-multi-column-list-option-card";
        } else {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"fed_files\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_fileslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_fileslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"btn btn-default " . $classname . "\" title=\"" . $Language->phrase("MultiColumnCards", true) . "\" data-ew-action=\"layout\" data-layout=\"cards\" data-url=\"" . $this->pageUrl() . "\">" . $Language->phrase("MultiColumnCards") . "</button>";
        $item->Visible = true;
        $item = &$this->LayoutOptions->add("table");
        $classname = $this->MultiColumnLayout == "table" ? " disabled" : "";
        $item->Body = "<button type=\"button\" class=\"btn btn-default " . $classname . "\" title=\"" . $Language->phrase("MultiColumnTable", true) . "\" data-ew-action=\"layout\" data-layout=\"table\" data-url=\"" . $this->pageUrl() . "\">" . $Language->phrase("MultiColumnTable") . "</button>";
        $item->Visible = true;
        $item = &$this->LayoutOptions->add("checkbox");
        $item->Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"ffed_fileslist\"><label class=\"form-check-label\" for=\"key\">" . $Language->phrase("SelectAll") . "</label></div></div>";
        $item->ShowInButtonGroup = true;
        $item->Visible = false && $this->MultiColumnLayout == "cards";
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffed_filessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffed_filessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffed_fileslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // thumbs
        if ($this->thumbs->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->thumbs->AdvancedSearch->SearchValue != "" || $this->thumbs->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if (is_array($this->type->AdvancedSearch->SearchValue)) {
            $this->type->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->type->AdvancedSearch->SearchValue);
        }
        if (is_array($this->type->AdvancedSearch->SearchValue2)) {
            $this->type->AdvancedSearch->SearchValue2 = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->type->AdvancedSearch->SearchValue2);
        }

        // dateUpdate
        if ($this->dateUpdate->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->dateUpdate->AdvancedSearch->SearchValue != "" || $this->dateUpdate->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // dateCreation
        if ($this->dateCreation->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->dateCreation->AdvancedSearch->SearchValue != "" || $this->dateCreation->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->thumbs->Raw) {
                $this->thumbs->AdvancedSearch->SearchValue = HtmlDecode($this->thumbs->AdvancedSearch->SearchValue);
            }
            $this->thumbs->EditValue = HtmlEncode($this->thumbs->AdvancedSearch->SearchValue);
            $this->thumbs->PlaceHolder = RemoveHtml($this->thumbs->caption());
            ...
            if ($curVal != "") {
                ...
                $this->category->AdvancedSearch->ViewValue = $this->category->Lookup !== null && is_array($this->category->lookupOptions()) ? $curVal : null;
            }
            if ($this->category->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($this->category->AdvancedSearch->ViewValue == "") {
                ...
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $this->category->AdvancedSearch->ViewValue = new OptionValues();
                    foreach ($rswrk as $row) {
                        $arwrk = $this->category->Lookup->renderViewRow($row);
                        $this->category->AdvancedSearch->ViewValue->add($this->category->displayValue($arwrk));
                        $ari++;
                    }
                } else {
                    $this->category->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
            ...
            if ($curVal != "") {
                ...
                $this->type->AdvancedSearch->ViewValue = $this->type->Lookup !== null && is_array($this->type->lookupOptions()) ? $curVal : null;
            }
            if ($this->type->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->type->EditValue = array_values($this->type->lookupOptions());
                if ($this->type->AdvancedSearch->ViewValue == "") {
                    $this->type->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                ...
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $this->type->AdvancedSearch->ViewValue = new OptionValues();
                    foreach ($rswrk as $row) {
                        $arwrk = $this->type->Lookup->renderViewRow($row);
                        $this->type->AdvancedSearch->ViewValue->add($this->type->displayValue($arwrk));
                        $ari++;
                    }
                } else {
                    $this->type->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffed_fileslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffed_filessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        // Example:
        $opt = &$this->ListOptions->Add("file-link-btn");
        $opt->Header = "Link";
        $opt->OnLeft = true;
        $opt->MoveTo(4); 
    }
    ...
    public function listOptionsRendered()
    {
        ...
        $link = '<a class="btn btn-primary" target = "_blank" href="'.$this->link->CurrentValue.'"><i class="fas fa-file"></i>    '.Language()->Phrase("see_file").' </a>';
        $this->ListOptions["file-link-btn"]->Body = $link;
    }
