 ...
 * Page class
 */
class FedMartialartsAdd extends FedMartialarts
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedMartialartsAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("FedMartialartsList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "FedMartialartsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "FedMartialartsView") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'martialArts' first before field var 'x_martialArts'
        $val = $CurrentForm->hasValue("martialArts") ? $CurrentForm->getValue("martialArts") : $CurrentForm->getValue("x_martialArts");
        if (!$this->martialArts->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->martialArts->Visible = false; // Disable update for API request
            } else {
                $this->martialArts->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->martialArts->CurrentValue = $this->martialArts->FormValue;
    ...
    public function renderRow()
    {
        ...
        // martialArts
        $this->martialArts->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->martialArts->EditValue = HtmlEncode($this->martialArts->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->martialArts->Required) {
            if (!$this->martialArts->IsDetailKey && EmptyValue($this->martialArts->FormValue)) {
                $this->martialArts->addErrorMessage(str_replace("%s", $this->martialArts->caption(), $this->martialArts->RequiredErrorMessage));
            }
        }
        ...
        if (in_array("fed_rank", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // martialArts
        $this->martialArts->setDbValueDef($rsnew, $this->martialArts->CurrentValue, null, false);
        ...
        if ($addRow) {
            ...
            $detailPage = Container("FedRankGrid");
            if (in_array("fed_rank", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->martialArtsId->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "fed_rank"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->martialArtsId->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
