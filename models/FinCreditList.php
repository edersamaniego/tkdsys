 ...
 * Page class
 */
class FinCreditList extends FinCredit
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'fin_credit';

    // Page object name
    public $PageObjName = "FinCreditList";
    ...
    public $FormName = "ffin_creditlist";
    ...
    public function __construct()
    {
        ...
        // Table object (fin_credit)
        if (!isset($GLOBALS["fin_credit"]) || get_class($GLOBALS["fin_credit"]) == PROJECT_NAMESPACE . "fin_credit") {
            $GLOBALS["fin_credit"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "FinCreditAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FinCreditDelete";
        $this->MultiUpdateUrl = "FinCreditUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'fin_credit');
        }
    ...
    public function run()
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
                if ($this->getCurrentMasterTable() == "fin_accountsreceivable") {
                    $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "fin_accountsreceivable"); // Add master User ID filter
                }
                if ($this->getCurrentMasterTable() == "fin_checkingaccount") {
                    $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "fin_checkingaccount"); // Add master User ID filter
                }
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fin_accountsreceivable") {
            $masterTbl = Container("fin_accountsreceivable");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("FinAccountsreceivableList"); // Return to master page
                return;
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fin_checkingaccount") {
            $masterTbl = Container("fin_checkingaccount");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("FinCheckingaccountList"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = ROWTYPE_MASTER; // Master row
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->dueDate); // dueDate
            $this->updateSort($this->value); // value
            $this->updateSort($this->paymentMethod); // paymentMethod
            $this->updateSort($this->checkingAccountId); // checkingAccountId
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffin_creditlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffin_creditlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("paymentMethod", $this->createColumnOption("paymentMethod"));
            $option->add("checkingAccountId", $this->createColumnOption("checkingAccountId"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffin_creditsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffin_creditsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffin_creditlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffin_creditlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fin_accountsreceivable") {
            $fin_accountsreceivable = new FinAccountsreceivableList();
            $rsmaster = $fin_accountsreceivable->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $fin_accountsreceivable;
                    $fin_accountsreceivable->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
        ...
        // Export master record
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fin_checkingaccount") {
            $fin_checkingaccount = new FinCheckingaccountList();
            $rsmaster = $fin_checkingaccount->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $fin_checkingaccount;
                    $fin_checkingaccount->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
                $doc->setStyle($exportStyle); // Restore
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "fin_accountsreceivable") {
                $validMaster = true;
                $masterTbl = Container("fin_accountsreceivable");
