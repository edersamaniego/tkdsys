...
/**
 * Table class for fin_type
 */
class FinType extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'fin_type';
        $this->TableName = 'fin_type';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`fin_type`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // defaultOrganization
        $this->defaultOrganization = new DbField(
            'fin_type',
            'fin_type',
            'x_defaultOrganization',
            'defaultOrganization',
            '`defaultOrganization`',
            '`defaultOrganization`',
            16,
        ...
        $this->defaultOrganization->InputTextType = "text";
        $this->defaultOrganization->DataType = DATATYPE_BOOLEAN;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->defaultOrganization->Lookup = new Lookup('defaultOrganization', 'fin_type', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
                break;
            case "pt-BR":
                $this->defaultOrganization->Lookup = new Lookup('defaultOrganization', 'fin_type', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
                break;
        ...
        $this->defaultOrganization->OptionCount = 2;
        $this->defaultOrganization->DefaultErrorMessage = $Language->phrase("IncorrectField");
        $this->Fields['defaultOrganization'] = &$this->defaultOrganization;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`fin_type`";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->defaultOrganization->DbValue = $row['defaultOrganization'];
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("FinTypeList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "FinTypeEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "FinTypeAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("FinTypeView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("FinTypeView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "FinTypeAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "FinTypeAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("FinTypeEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("FinTypeAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("FinTypeDelete", $this->getUrlParm());
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->masterSchoolId);
                    $doc->exportCaption($this->organization);
                    $doc->exportCaption($this->defaultOrganization);
                }
                $doc->endExportRow();
            }
        }
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->masterSchoolId);
                        $doc->exportField($this->organization);
                        $doc->exportField($this->defaultOrganization);
                    }
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM `fin_type`";
    ...
    public function rowInserting($rsold, &$rsnew)
    {
       ...
       // $rsnew['organization'] = CurrentOrganizationID();
        return true;
    }
