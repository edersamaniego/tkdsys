 ...
 * Page class
 */
class FedVideosubsectionList extends FedVideosubsection
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'fed_videosubsection';

    // Page object name
    public $PageObjName = "FedVideosubsectionList";
    ...
    public $FormName = "ffed_videosubsectionlist";
    ...
    public function __construct()
    {
        ...
        // Table object (fed_videosubsection)
        if (!isset($GLOBALS["fed_videosubsection"]) || get_class($GLOBALS["fed_videosubsection"]) == PROJECT_NAMESPACE . "fed_videosubsection") {
            $GLOBALS["fed_videosubsection"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "FedVideosubsectionAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FedVideosubsectionDelete";
        $this->MultiUpdateUrl = "FedVideosubsectionUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'fed_videosubsection');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("fed_videosubsection");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->subsectionBr->setVisibility();
        $this->subsectionSp->setVisibility();
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fed_videosection") {
            $masterTbl = Container("fed_videosection");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("FedVideosectionList"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffed_videosubsectionsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->section->AdvancedSearch->toJson(), ","); // Field section
        $filterList = Concat($filterList, $this->subsection->AdvancedSearch->toJson(), ","); // Field subsection
        $filterList = Concat($filterList, $this->subsectionBr->AdvancedSearch->toJson(), ","); // Field subsectionBr
        $filterList = Concat($filterList, $this->subsectionSp->AdvancedSearch->toJson(), ","); // Field subsectionSp
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffed_videosubsectionsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field subsection
        $this->subsection->AdvancedSearch->SearchValue = @$filter["x_subsection"];
        $this->subsection->AdvancedSearch->SearchOperator = @$filter["z_subsection"];
        $this->subsection->AdvancedSearch->SearchCondition = @$filter["v_subsection"];
        $this->subsection->AdvancedSearch->SearchValue2 = @$filter["y_subsection"];
        $this->subsection->AdvancedSearch->SearchOperator2 = @$filter["w_subsection"];
        $this->subsection->AdvancedSearch->save();

        // Field subsectionBr
        $this->subsectionBr->AdvancedSearch->SearchValue = @$filter["x_subsectionBr"];
        $this->subsectionBr->AdvancedSearch->SearchOperator = @$filter["z_subsectionBr"];
        $this->subsectionBr->AdvancedSearch->SearchCondition = @$filter["v_subsectionBr"];
        $this->subsectionBr->AdvancedSearch->SearchValue2 = @$filter["y_subsectionBr"];
        $this->subsectionBr->AdvancedSearch->SearchOperator2 = @$filter["w_subsectionBr"];
        $this->subsectionBr->AdvancedSearch->save();

        // Field subsectionSp
        $this->subsectionSp->AdvancedSearch->SearchValue = @$filter["x_subsectionSp"];
        $this->subsectionSp->AdvancedSearch->SearchOperator = @$filter["z_subsectionSp"];
        $this->subsectionSp->AdvancedSearch->SearchCondition = @$filter["v_subsectionSp"];
        $this->subsectionSp->AdvancedSearch->SearchValue2 = @$filter["y_subsectionSp"];
        $this->subsectionSp->AdvancedSearch->SearchOperator2 = @$filter["w_subsectionSp"];
        $this->subsectionSp->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->subsection;
        $searchFlds[] = &$this->subsectionBr;
        $searchFlds[] = &$this->subsectionSp;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->subsection); // subsection
            $this->updateSort($this->subsectionBr); // subsectionBr
            $this->updateSort($this->subsectionSp); // subsectionSp
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->subsection->setSort("");
                $this->subsectionBr->setSort("");
                $this->subsectionSp->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_videosubsectionlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_videosubsectionlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'fed_video')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("FedVideoList?" . Config("TABLE_SHOW_MASTER") . "=fed_videosubsection&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("FedVideoGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fed_videosubsection')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fed_videosubsection')) {
            ...
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'fed_videosubsection')) {
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Preview extension
        $links = "";
        $sqlwrk = "`subsection`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";
        ...
        if ($this->DetailPages && $this->DetailPages["fed_video"] && $this->DetailPages["fed_video"]->Visible && $Security->allowList(CurrentProjectID() . 'fed_video')) {
            ...
            $url = "FedVideoPreview?t=fed_videosubsection&f=" . Encrypt($sqlwrk);
            ...
            if ($Security->allowList(CurrentProjectID() . 'fed_videosubsection')) {
                ...
                $detaillnk = GetUrl("FedVideoList?" . Config("TABLE_SHOW_MASTER") . "=fed_videosubsection&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fed_videosubsection')) {
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fed_videosubsection')) {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'fed_videosubsection') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("subsection", $this->createColumnOption("subsection"));
            $option->add("subsectionBr", $this->createColumnOption("subsectionBr"));
            $option->add("subsectionSp", $this->createColumnOption("subsectionSp"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffed_videosubsectionsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffed_videosubsectionsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffed_videosubsectionlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->subsectionBr->setDbValue($row['subsectionBr']);
        $this->subsectionSp->setDbValue($row['subsectionSp']);
    ...
    protected function newRow()
    {
        ...
        $row['subsectionBr'] = $this->subsectionBr->DefaultValue;
        $row['subsectionSp'] = $this->subsectionSp->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // subsection
            $this->subsection->ViewValue = $this->subsection->CurrentValue;
            $this->subsection->ViewCustomAttributes = "";

            // subsectionBr
            $this->subsectionBr->ViewValue = $this->subsectionBr->CurrentValue;
            $this->subsectionBr->ViewCustomAttributes = "";

            // subsectionSp
            $this->subsectionSp->ViewValue = $this->subsectionSp->CurrentValue;
            $this->subsectionSp->ViewCustomAttributes = "";
            ...
            // subsectionBr
            $this->subsectionBr->LinkCustomAttributes = "";
            $this->subsectionBr->HrefValue = "";
            $this->subsectionBr->TooltipValue = "";

            // subsectionSp
            $this->subsectionSp->LinkCustomAttributes = "";
            $this->subsectionSp->HrefValue = "";
            $this->subsectionSp->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffed_videosubsectionlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffed_videosubsectionsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fed_videosection") {
            $fed_videosection = new FedVideosectionList();
            $rsmaster = $fed_videosection->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $fed_videosection;
                    $fed_videosection->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
