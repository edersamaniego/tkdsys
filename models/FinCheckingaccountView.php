 ...
 * Page class
 */
class FinCheckingaccountView extends FinCheckingaccount
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FinCheckingaccountView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            ...
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "FinCheckingaccountList"; // Return to list
            }
                ...
                case "show": // Get a record to display
                    if (!$this->IsModal) { // Normal view page
                        ...
                        if ($this->TotalRecords <= 0) { // No record found
                            ...
                            $this->terminate("FinCheckingaccountList"); // Return to list page
                            return;
                        ...
                        if (!$matchRecord) {
                            ...
                            $returnUrl = "FinCheckingaccountList"; // No matching record, return to list
                        } else {
            ...
            $returnUrl = "FinCheckingaccountList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        // "detail_fin_debit"
        $item = &$option->add("detail_fin_debit");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("fin_debit", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("FinDebitList?" . Config("TABLE_SHOW_MASTER") . "=fin_checkingaccount&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("FinDebitGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fin_checkingaccount')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fin_debit"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "fin_debit";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fin_checkingaccount')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fin_debit"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "fin_debit";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'fin_checkingaccount')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fin_debit"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
        ...
        // "detail_fin_credit"
        $item = &$option->add("detail_fin_credit");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("fin_credit", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("FinCreditList?" . Config("TABLE_SHOW_MASTER") . "=fin_checkingaccount&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("FinCreditGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fin_checkingaccount')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fin_credit"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fin_checkingaccount')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fin_credit"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'fin_checkingaccount')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fin_credit"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    */
    public function exportData($return = false)
    {
        ...
        // Export detail records (fin_debit)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("fin_debit", explode(",", $this->getCurrentDetailTable()))) {
            $fin_debit = new FinDebitList();
            $rsdetail = $fin_debit->loadRs($fin_debit->getDetailFilterFromSession()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->Table;
                    $doc->Table = $fin_debit;
                    $fin_debit->exportDocument($doc, new Recordset($rsdetail), 1, $detailcnt);
                    $doc->Table = $oldtbl;
                }
        ...
        // Export detail records (fin_credit)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("fin_credit", explode(",", $this->getCurrentDetailTable()))) {
            $fin_credit = new FinCreditList();
            $rsdetail = $fin_credit->loadRs($fin_credit->getDetailFilterFromSession()); // Load detail records
            if ($rsdetail) {
                $exportStyle = $doc->Style;
                $doc->setStyle("h"); // Change to horizontal
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $detailcnt = $rsdetail->rowCount();
                    $oldtbl = $doc->Table;
                    $doc->Table = $fin_credit;
                    $fin_credit->exportDocument($doc, new Recordset($rsdetail), 1, $detailcnt);
                    $doc->Table = $oldtbl;
                }
                $doc->setStyle($exportStyle); // Restore
            }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("FinCheckingaccountList"), "", $this->TableVar, true);
        $pageId = "view";
