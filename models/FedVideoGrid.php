 ...
 * Page class
 */
class FedVideoGrid extends FedVideo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedVideoGrid";
    ...
    public $FormName = "ffed_videogrid";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fed_videosubsection") {
            $masterTbl = Container("fed_videosubsection");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("FedVideosubsectionList"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = ROWTYPE_MASTER; // Master row
    ...
    public function emptyRow()
    {
        ...
        if (
            ...
            !($this->_title->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->_title->CurrentValue == $this->_title->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_URL") &&
            $CurrentForm->hasValue("o_URL") &&
            $this->URL->CurrentValue != $this->URL->DefaultValue &&
            !($this->URL->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->URL->CurrentValue == $this->URL->getSessionValue())
        ) {
            return false;
        }
        if (
        ...
        if (
            $CurrentForm->hasValue("x_createDate") &&
            $CurrentForm->hasValue("o_createDate") &&
            $this->createDate->CurrentValue != $this->createDate->DefaultValue &&
            !($this->createDate->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->createDate->CurrentValue == $this->createDate->getSessionValue())
        ) {
            return false;
        }
        return true;
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"fed_video\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
    ...
    {
        $this->thumbs->Upload->Index = $this->RowIndex;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_URL")) {
            $this->URL->setOldValue($CurrentForm->getValue("o_URL"));
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->subsection->getSessionValue() != "") {
                ...
                $this->subsection->OldValue = $this->subsection->CurrentValue;
                $curVal = strval($this->subsection->CurrentValue);
                if ($curVal != "") {
                    $this->subsection->ViewValue = $this->subsection->lookupCacheOption($curVal);
                    if ($this->subsection->ViewValue === null) { // Lookup from database
