 ...
 * Page class
 */
class ConfTesttypeList extends ConfTesttype
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'conf_testtype';

    // Page object name
    public $PageObjName = "ConfTesttypeList";
    ...
    public $FormName = "fconf_testtypelist";
    ...
    public function __construct()
    {
        ...
        // Table object (conf_testtype)
        if (!isset($GLOBALS["conf_testtype"]) || get_class($GLOBALS["conf_testtype"]) == PROJECT_NAMESPACE . "conf_testtype") {
            $GLOBALS["conf_testtype"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ConfTesttypeAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ConfTesttypeDelete";
        $this->MultiUpdateUrl = "ConfTesttypeUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'conf_testtype');
        }
    ...
    public function run()
    {
        ...
        $this->testTypeEN->setVisibility();
        $this->testTypeES->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fconf_testtypesrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->testType->AdvancedSearch->toJson(), ","); // Field testType
        $filterList = Concat($filterList, $this->testTypeEN->AdvancedSearch->toJson(), ","); // Field testTypeEN
        $filterList = Concat($filterList, $this->testTypeES->AdvancedSearch->toJson(), ","); // Field testTypeES
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fconf_testtypesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field testTypeEN
        $this->testTypeEN->AdvancedSearch->SearchValue = @$filter["x_testTypeEN"];
        $this->testTypeEN->AdvancedSearch->SearchOperator = @$filter["z_testTypeEN"];
        $this->testTypeEN->AdvancedSearch->SearchCondition = @$filter["v_testTypeEN"];
        $this->testTypeEN->AdvancedSearch->SearchValue2 = @$filter["y_testTypeEN"];
        $this->testTypeEN->AdvancedSearch->SearchOperator2 = @$filter["w_testTypeEN"];
        $this->testTypeEN->AdvancedSearch->save();

        // Field testTypeES
        $this->testTypeES->AdvancedSearch->SearchValue = @$filter["x_testTypeES"];
        $this->testTypeES->AdvancedSearch->SearchOperator = @$filter["z_testTypeES"];
        $this->testTypeES->AdvancedSearch->SearchCondition = @$filter["v_testTypeES"];
        $this->testTypeES->AdvancedSearch->SearchValue2 = @$filter["y_testTypeES"];
        $this->testTypeES->AdvancedSearch->SearchOperator2 = @$filter["w_testTypeES"];
        $this->testTypeES->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->testType;
        $searchFlds[] = &$this->testTypeEN;
        $searchFlds[] = &$this->testTypeES;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->testTypeEN); // testTypeEN
            $this->updateSort($this->testTypeES); // testTypeES
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->testTypeEN->setSort("");
                $this->testTypeES->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fconf_testtypelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fconf_testtypelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("testType", $this->createColumnOption("testType"));
            $option->add("testTypeEN", $this->createColumnOption("testTypeEN"));
            $option->add("testTypeES", $this->createColumnOption("testTypeES"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fconf_testtypesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fconf_testtypesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fconf_testtypelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->testTypeEN->setDbValue($row['testTypeEN']);
        $this->testTypeES->setDbValue($row['testTypeES']);
    ...
    protected function newRow()
    {
        ...
        $row['testType'] = $this->testType->DefaultValue;
        $row['testTypeEN'] = $this->testTypeEN->DefaultValue;
        $row['testTypeES'] = $this->testTypeES->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // testTypeEN
            $this->testTypeEN->ViewValue = $this->testTypeEN->CurrentValue;
            $this->testTypeEN->ViewCustomAttributes = "";

            // testTypeES
            $this->testTypeES->ViewValue = $this->testTypeES->CurrentValue;
            $this->testTypeES->ViewCustomAttributes = "";
            ...
            // testTypeEN
            $this->testTypeEN->LinkCustomAttributes = "";
            $this->testTypeEN->HrefValue = "";
            $this->testTypeEN->TooltipValue = "";

            // testTypeES
            $this->testTypeES->LinkCustomAttributes = "";
            $this->testTypeES->HrefValue = "";
            $this->testTypeES->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fconf_testtypelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fconf_testtypesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
