 ...
 * Page class
 */
class FedFilescategoryEdit extends FedFilescategory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedFilescategoryEdit";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "FedFilescategoryView") {
                        $row["view"] = "1";
                    }
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("FedFilescategoryList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("FedFilescategoryList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "FedFilescategoryList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'categoryBr' first before field var 'x_categoryBr'
        $val = $CurrentForm->hasValue("categoryBr") ? $CurrentForm->getValue("categoryBr") : $CurrentForm->getValue("x_categoryBr");
        if (!$this->categoryBr->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->categoryBr->Visible = false; // Disable update for API request
            } else {
                $this->categoryBr->setFormValue($val);
            }
        }

        // Check field name 'categorySp' first before field var 'x_categorySp'
        $val = $CurrentForm->hasValue("categorySp") ? $CurrentForm->getValue("categorySp") : $CurrentForm->getValue("x_categorySp");
        if (!$this->categorySp->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->categorySp->Visible = false; // Disable update for API request
            } else {
                $this->categorySp->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->categoryBr->CurrentValue = $this->categoryBr->FormValue;
        $this->categorySp->CurrentValue = $this->categorySp->FormValue;
    ...
    public function renderRow()
    {
        ...
        // categoryBr
        $this->categoryBr->RowCssClass = "row";

        // categorySp
        $this->categorySp->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // categoryBr
            $this->categoryBr->setupEditAttributes();
            $this->categoryBr->EditCustomAttributes = "";
            if (!$this->categoryBr->Raw) {
                $this->categoryBr->CurrentValue = HtmlDecode($this->categoryBr->CurrentValue);
            }
            $this->categoryBr->EditValue = HtmlEncode($this->categoryBr->CurrentValue);
            $this->categoryBr->PlaceHolder = RemoveHtml($this->categoryBr->caption());

            // categorySp
            $this->categorySp->setupEditAttributes();
            $this->categorySp->EditCustomAttributes = "";
            if (!$this->categorySp->Raw) {
                $this->categorySp->CurrentValue = HtmlDecode($this->categorySp->CurrentValue);
            }
            $this->categorySp->EditValue = HtmlEncode($this->categorySp->CurrentValue);
            $this->categorySp->PlaceHolder = RemoveHtml($this->categorySp->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->categoryBr->Required) {
            if (!$this->categoryBr->IsDetailKey && EmptyValue($this->categoryBr->FormValue)) {
                $this->categoryBr->addErrorMessage(str_replace("%s", $this->categoryBr->caption(), $this->categoryBr->RequiredErrorMessage));
            }
        }
        if ($this->categorySp->Required) {
            if (!$this->categorySp->IsDetailKey && EmptyValue($this->categorySp->FormValue)) {
                $this->categorySp->addErrorMessage(str_replace("%s", $this->categorySp->caption(), $this->categorySp->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // categoryBr
        $this->categoryBr->setDbValueDef($rsnew, $this->categoryBr->CurrentValue, null, $this->categoryBr->ReadOnly);

        // categorySp
        $this->categorySp->setDbValueDef($rsnew, $this->categorySp->CurrentValue, null, $this->categorySp->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("FedFilescategoryList"), "", $this->TableVar, true);
        $pageId = "edit";
