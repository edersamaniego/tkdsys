 ...
 * Page class
 */
class FedRankList extends FedRank
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedRankList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "FedRankAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FedRankDelete";
        $this->MultiUpdateUrl = "FedRankUpdate";
    ...
    public function run()
    {
        ...
        $this->levelTournamentId->Visible = false;
        $this->martialArtsId->Visible = false;
        $this->createUserId->Visible = false;
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fed_martialarts") {
            $masterTbl = Container("fed_martialarts");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("FedMartialartsList"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffed_ranksrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->rankBR->AdvancedSearch->toJson(), ","); // Field rankBR
        $filterList = Concat($filterList, $this->rankUS->AdvancedSearch->toJson(), ","); // Field rankUS
        $filterList = Concat($filterList, $this->rankES->AdvancedSearch->toJson(), ","); // Field rankES
        $filterList = Concat($filterList, $this->ranking->AdvancedSearch->toJson(), ","); // Field ranking
        $filterList = Concat($filterList, $this->nextrankId->AdvancedSearch->toJson(), ","); // Field nextrankId
        $filterList = Concat($filterList, $this->levelTournamentId->AdvancedSearch->toJson(), ","); // Field levelTournamentId
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffed_ranksrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field rankBR
        $this->rankBR->AdvancedSearch->SearchValue = @$filter["x_rankBR"];
        $this->rankBR->AdvancedSearch->SearchOperator = @$filter["z_rankBR"];
        $this->rankBR->AdvancedSearch->SearchCondition = @$filter["v_rankBR"];
        $this->rankBR->AdvancedSearch->SearchValue2 = @$filter["y_rankBR"];
        $this->rankBR->AdvancedSearch->SearchOperator2 = @$filter["w_rankBR"];
        $this->rankBR->AdvancedSearch->save();

        // Field rankUS
        $this->rankUS->AdvancedSearch->SearchValue = @$filter["x_rankUS"];
        $this->rankUS->AdvancedSearch->SearchOperator = @$filter["z_rankUS"];
        $this->rankUS->AdvancedSearch->SearchCondition = @$filter["v_rankUS"];
        $this->rankUS->AdvancedSearch->SearchValue2 = @$filter["y_rankUS"];
        $this->rankUS->AdvancedSearch->SearchOperator2 = @$filter["w_rankUS"];
        $this->rankUS->AdvancedSearch->save();

        // Field rankES
        $this->rankES->AdvancedSearch->SearchValue = @$filter["x_rankES"];
        $this->rankES->AdvancedSearch->SearchOperator = @$filter["z_rankES"];
        $this->rankES->AdvancedSearch->SearchCondition = @$filter["v_rankES"];
        $this->rankES->AdvancedSearch->SearchValue2 = @$filter["y_rankES"];
        $this->rankES->AdvancedSearch->SearchOperator2 = @$filter["w_rankES"];
        $this->rankES->AdvancedSearch->save();
        ...
        // Field levelTournamentId
        $this->levelTournamentId->AdvancedSearch->SearchValue = @$filter["x_levelTournamentId"];
        $this->levelTournamentId->AdvancedSearch->SearchOperator = @$filter["z_levelTournamentId"];
        $this->levelTournamentId->AdvancedSearch->SearchCondition = @$filter["v_levelTournamentId"];
        $this->levelTournamentId->AdvancedSearch->SearchValue2 = @$filter["y_levelTournamentId"];
        $this->levelTournamentId->AdvancedSearch->SearchOperator2 = @$filter["w_levelTournamentId"];
        $this->levelTournamentId->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->rankBR, $default, false); // rankBR
        $this->buildSearchSql($where, $this->rankUS, $default, false); // rankUS
        $this->buildSearchSql($where, $this->rankES, $default, false); // rankES
        $this->buildSearchSql($where, $this->ranking, $default, false); // ranking
        $this->buildSearchSql($where, $this->nextrankId, $default, false); // nextrankId
        $this->buildSearchSql($where, $this->levelTournamentId, $default, false); // levelTournamentId
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->rankBR->AdvancedSearch->save(); // rankBR
            $this->rankUS->AdvancedSearch->save(); // rankUS
            $this->rankES->AdvancedSearch->save(); // rankES
            $this->ranking->AdvancedSearch->save(); // ranking
            $this->nextrankId->AdvancedSearch->save(); // nextrankId
            $this->levelTournamentId->AdvancedSearch->save(); // levelTournamentId
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->rankBR;
        $searchFlds[] = &$this->rankUS;
        $searchFlds[] = &$this->rankES;
        $searchFlds[] = &$this->ranking;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->rankBR->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->rankUS->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->rankES->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->levelTournamentId->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->rankBR->AdvancedSearch->unsetSession();
        $this->rankUS->AdvancedSearch->unsetSession();
        $this->rankES->AdvancedSearch->unsetSession();
        $this->ranking->AdvancedSearch->unsetSession();
        $this->nextrankId->AdvancedSearch->unsetSession();
        $this->levelTournamentId->AdvancedSearch->unsetSession();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->rankBR); // rankBR
            $this->updateSort($this->rankUS); // rankUS
            $this->updateSort($this->rankES); // rankES
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetall") {
                ...
                $this->DbMasterFilter = "";
                $this->DbDetailFilter = "";
                        $this->martialArtsId->setSessionValue("");
            }
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->levelTournamentId->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_ranklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_ranklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("rankBR", $this->createColumnOption("rankBR"));
            $option->add("rankUS", $this->createColumnOption("rankUS"));
            $option->add("rankES", $this->createColumnOption("rankES"));
            $option->add("ranking", $this->createColumnOption("ranking"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffed_ranksrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffed_ranksrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffed_ranklist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->rankBR->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->rankBR->AdvancedSearch->SearchValue != "" || $this->rankBR->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->rankUS->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->rankUS->AdvancedSearch->SearchValue != "" || $this->rankUS->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->rankES->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->rankES->AdvancedSearch->SearchValue != "" || $this->rankES->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ranking->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ranking->AdvancedSearch->SearchValue != "" || $this->ranking->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->levelTournamentId->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->levelTournamentId->AdvancedSearch->SearchValue != "" || $this->levelTournamentId->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->rankBR->setDbValue($row['rankBR']);
        $this->rankUS->setDbValue($row['rankUS']);
        $this->rankES->setDbValue($row['rankES']);
        $this->ranking->setDbValue($row['ranking']);
        $this->nextrankId->setDbValue($row['nextrankId']);
        $this->levelTournamentId->setDbValue($row['levelTournamentId']);
    ...
    protected function newRow()
    {
        ...
        $row['rankBR'] = $this->rankBR->DefaultValue;
        $row['rankUS'] = $this->rankUS->DefaultValue;
        $row['rankES'] = $this->rankES->DefaultValue;
        $row['ranking'] = $this->ranking->DefaultValue;
        $row['nextrankId'] = $this->nextrankId->DefaultValue;
        $row['levelTournamentId'] = $this->levelTournamentId->DefaultValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffed_ranklist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffed_ranksrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"FedRankSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "fed_martialarts") {
            $fed_martialarts = new FedMartialartsList();
            $rsmaster = $fed_martialarts->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $fed_martialarts;
                    $fed_martialarts->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "fed_martialarts") {
                $validMaster = true;
                $masterTbl = Container("fed_martialarts");
                if (($parm = Get("fk_id", Get("martialArtsId"))) !== null) {
                    $masterTbl->id->setQueryStringValue($parm);
                    $this->martialArtsId->setQueryStringValue($masterTbl->id->QueryStringValue);
                    $this->martialArtsId->setSessionValue($this->martialArtsId->QueryStringValue);
            ...
            if ($masterTblVar == "fed_martialarts") {
                $validMaster = true;
                $masterTbl = Container("fed_martialarts");
                if (($parm = Post("fk_id", Post("martialArtsId"))) !== null) {
                    $masterTbl->id->setFormValue($parm);
                    $this->martialArtsId->setFormValue($masterTbl->id->FormValue);
                    $this->martialArtsId->setSessionValue($this->martialArtsId->FormValue);
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "fed_martialarts") {
                if ($this->martialArtsId->CurrentValue == "") {
                    $this->martialArtsId->setSessionValue("");
                }
            }
        }
