 ...
 * Page class
 */
class FedMartialartsList extends FedMartialarts
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'fed_martialarts';

    // Page object name
    public $PageObjName = "FedMartialartsList";
    ...
    public $FormName = "ffed_martialartslist";
    ...
    public function __construct()
    {
        ...
        // Table object (fed_martialarts)
        if (!isset($GLOBALS["fed_martialarts"]) || get_class($GLOBALS["fed_martialarts"]) == PROJECT_NAMESPACE . "fed_martialarts") {
            $GLOBALS["fed_martialarts"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "FedMartialartsAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FedMartialartsDelete";
        $this->MultiUpdateUrl = "FedMartialartsUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'fed_martialarts');
        }
    ...
    public function run()
    {
        ...
        $this->martialArts->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffed_martialartssrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->martialArts->AdvancedSearch->toJson(), ","); // Field martialArts
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffed_martialartssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field martialArts
        $this->martialArts->AdvancedSearch->SearchValue = @$filter["x_martialArts"];
        $this->martialArts->AdvancedSearch->SearchOperator = @$filter["z_martialArts"];
        $this->martialArts->AdvancedSearch->SearchCondition = @$filter["v_martialArts"];
        $this->martialArts->AdvancedSearch->SearchValue2 = @$filter["y_martialArts"];
        $this->martialArts->AdvancedSearch->SearchOperator2 = @$filter["w_martialArts"];
        $this->martialArts->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->martialArts); // martialArts
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_fed_rank"
        $item = &$this->ListOptions->add("detail_fed_rank");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'fed_rank');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_martialartslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_martialartslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_fed_rank"
        $opt = $this->ListOptions["detail_fed_rank"];
        if ($Security->allowList(CurrentProjectID() . 'fed_rank')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("fed_rank", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("FedRankList?" . Config("TABLE_SHOW_MASTER") . "=fed_martialarts&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("FedRankGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fed_martialarts')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "fed_rank";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fed_martialarts')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank");
                ...
                $detailEditTblVar .= "fed_rank";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'fed_martialarts')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank");
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Preview extension
        $links = "";
        $sqlwrk = "`martialArtsId`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";

        // Column "detail_fed_rank"
        if ($this->DetailPages && $this->DetailPages["fed_rank"] && $this->DetailPages["fed_rank"]->Visible && $Security->allowList(CurrentProjectID() . 'fed_rank')) {
            $link = "";
            $option = $this->ListOptions["detail_fed_rank"];
            $url = "FedRankPreview?t=fed_martialarts&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"fed_rank\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'fed_martialarts')) {
                $label = $Language->TablePhrase("fed_rank", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"fed_rank\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("FedRankList?" . Config("TABLE_SHOW_MASTER") . "=fed_martialarts&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $title = $Language->TablePhrase("fed_rank", "TblCaption");
            ...
            $detailPageObj = Container("FedRankGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fed_martialarts')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fed_martialarts')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'fed_martialarts')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            $btngrp .= "</div>";
            if ($link != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_fed_rank");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank");
                $detailPage = Container("FedRankGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'fed_martialarts') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "fed_rank";
                }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("martialArts", $this->createColumnOption("martialArts"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffed_martialartssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffed_martialartssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffed_martialartslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->martialArts->setDbValue($row['martialArts']);
    ...
    protected function newRow()
    {
        ...
        $row['martialArts'] = $this->martialArts->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // martialArts
            $this->martialArts->ViewValue = $this->martialArts->CurrentValue;
            $this->martialArts->ViewCustomAttributes = "";
            ...
            // martialArts
            $this->martialArts->LinkCustomAttributes = "";
            $this->martialArts->HrefValue = "";
            $this->martialArts->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffed_martialartslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffed_martialartssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
