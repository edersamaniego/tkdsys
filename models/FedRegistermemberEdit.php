 ...
 * Page class
 */
class FedRegistermemberEdit extends FedRegistermember
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'fed_registermember';

    // Page object name
    public $PageObjName = "FedRegistermemberEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (fed_registermember)
        if (!isset($GLOBALS["fed_registermember"]) || get_class($GLOBALS["fed_registermember"]) == PROJECT_NAMESPACE . "fed_registermember") {
            $GLOBALS["fed_registermember"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'fed_registermember');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("fed_registermember");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->currentRankId->setVisibility();
        $this->obs->setVisibility();
        $this->registerType->setVisibility();
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("FedRegistermemberList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("FedRegistermemberList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "FedRegistermemberList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'currentRankId' first before field var 'x_currentRankId'
        $val = $CurrentForm->hasValue("currentRankId") ? $CurrentForm->getValue("currentRankId") : $CurrentForm->getValue("x_currentRankId");
        if (!$this->currentRankId->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->currentRankId->Visible = false; // Disable update for API request
            } else {
                $this->currentRankId->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'registerType' first before field var 'x_registerType'
        $val = $CurrentForm->hasValue("registerType") ? $CurrentForm->getValue("registerType") : $CurrentForm->getValue("x_registerType");
        if (!$this->registerType->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->registerType->Visible = false; // Disable update for API request
            } else {
                $this->registerType->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->currentRankId->CurrentValue = $this->currentRankId->FormValue;
        $this->obs->CurrentValue = $this->obs->FormValue;
        $this->registerType->CurrentValue = $this->registerType->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['currentRankId'] = $this->currentRankId->DefaultValue;
        $row['obs'] = $this->obs->DefaultValue;
        $row['registerType'] = $this->registerType->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        // currentRankId
        $this->currentRankId->RowCssClass = "row";

        // obs
        $this->obs->RowCssClass = "row";

        // registerType
        $this->registerType->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->currentRankId->EditValue = HtmlEncode($this->currentRankId->CurrentValue);
            ...
            $this->registerType->EditValue = HtmlEncode($this->registerType->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->currentRankId->Required) {
            if (!$this->currentRankId->IsDetailKey && EmptyValue($this->currentRankId->FormValue)) {
                $this->currentRankId->addErrorMessage(str_replace("%s", $this->currentRankId->caption(), $this->currentRankId->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->currentRankId->FormValue)) {
            $this->currentRankId->addErrorMessage($this->currentRankId->getErrorMessage(false));
        }
        ...
        if ($this->registerType->Required) {
            if (!$this->registerType->IsDetailKey && EmptyValue($this->registerType->FormValue)) {
                $this->registerType->addErrorMessage(str_replace("%s", $this->registerType->caption(), $this->registerType->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->registerType->FormValue)) {
            $this->registerType->addErrorMessage($this->registerType->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        // currentRankId
        $this->currentRankId->setDbValueDef($rsnew, $this->currentRankId->CurrentValue, null, $this->currentRankId->ReadOnly);

        // obs
        $this->obs->setDbValueDef($rsnew, $this->obs->CurrentValue, null, $this->obs->ReadOnly);

        // registerType
        $this->registerType->setDbValueDef($rsnew, $this->registerType->CurrentValue, null, $this->registerType->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("FedRegistermemberList"), "", $this->TableVar, true);
        $pageId = "edit";
