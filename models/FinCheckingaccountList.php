 ...
 * Page class
 */
class FinCheckingaccountList extends FinCheckingaccount
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'fin_checkingaccount';

    // Page object name
    public $PageObjName = "FinCheckingaccountList";
    ...
    public $FormName = "ffin_checkingaccountlist";
    ...
    public function __construct()
    {
        ...
        // Table object (fin_checkingaccount)
        if (!isset($GLOBALS["fin_checkingaccount"]) || get_class($GLOBALS["fin_checkingaccount"]) == PROJECT_NAMESPACE . "fin_checkingaccount") {
            $GLOBALS["fin_checkingaccount"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "FinCheckingaccountAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FinCheckingaccountDelete";
        $this->MultiUpdateUrl = "FinCheckingaccountUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'fin_checkingaccount');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("fin_checkingaccount");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffin_checkingaccountsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->bank->AdvancedSearch->toJson(), ","); // Field bank
        $filterList = Concat($filterList, $this->responsable->AdvancedSearch->toJson(), ","); // Field responsable
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffin_checkingaccountsrch", $filters);
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->bank;
        $searchFlds[] = &$this->responsable;
        $searchFlds[] = &$this->obs;
        $searchFlds[] = &$this->telephone;
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffin_checkingaccountlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffin_checkingaccountlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'fin_debit') && $this->showOptionLink()) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("FinDebitList?" . Config("TABLE_SHOW_MASTER") . "=fin_checkingaccount&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("FinDebitGrid");
            if ($detailPage->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'fin_checkingaccount')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'fin_checkingaccount')) {
            ...
            if ($detailPage->DetailAdd && $Security->canAdd() && $this->showOptionLink("add") && $Security->allowAdd(CurrentProjectID() . 'fin_checkingaccount')) {
        ...
        if ($Security->allowList(CurrentProjectID() . 'fin_credit') && $this->showOptionLink()) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("FinCreditList?" . Config("TABLE_SHOW_MASTER") . "=fin_checkingaccount&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages && $this->DetailPages["fin_debit"] && $this->DetailPages["fin_debit"]->Visible && $Security->allowList(CurrentProjectID() . 'fin_debit')) {
            ...
            $url = "FinDebitPreview?t=fin_checkingaccount&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"fin_debit\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'fin_checkingaccount')) {
                ...
                $detaillnk = GetUrl("FinDebitList?" . Config("TABLE_SHOW_MASTER") . "=fin_checkingaccount&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'fin_checkingaccount')) {
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'fin_checkingaccount')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fin_debit"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailAdd && $Security->canAdd() && $this->showOptionLink("add") && $Security->allowAdd(CurrentProjectID() . 'fin_checkingaccount')) {
        ...
        if ($this->DetailPages && $this->DetailPages["fin_credit"] && $this->DetailPages["fin_credit"]->Visible && $Security->allowList(CurrentProjectID() . 'fin_credit')) {
            ...
            $url = "FinCreditPreview?t=fin_checkingaccount&f=" . Encrypt($sqlwrk);
            ...
            if ($Security->allowList(CurrentProjectID() . 'fin_checkingaccount')) {
                ...
                $detaillnk = GetUrl("FinCreditList?" . Config("TABLE_SHOW_MASTER") . "=fin_checkingaccount&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'fin_checkingaccount') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("bank", $this->createColumnOption("bank"));
            $option->add("responsable", $this->createColumnOption("responsable"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffin_checkingaccountsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffin_checkingaccountsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffin_checkingaccountlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        ...
        $row['bank'] = $this->bank->DefaultValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffin_checkingaccountsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
