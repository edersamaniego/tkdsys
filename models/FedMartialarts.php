...
/**
 * Table class for fed_martialarts
 */
class FedMartialarts extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'fed_martialarts';
        $this->TableName = 'fed_martialarts';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`fed_martialarts`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->martialArts->InputTextType = "text";
        $this->Fields['martialArts'] = &$this->martialArts;
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "fed_rank") {
            $detailUrl = Container("fed_rank")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "FedMartialartsList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`fed_martialarts`";
    }
    ...
    {
        // Cascade Update detail table 'fed_rank'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['id']) && $rsold['id'] != $rs['id'])) { // Update detail field 'martialArtsId'
            $cascadeUpdate = true;
            $rscascade['martialArtsId'] = $rs['id'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("fed_rank")->loadRs("`martialArtsId` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("fed_rank")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("fed_rank")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("fed_rank")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'fed_rank'
        $dtlrows = Container("fed_rank")->loadRs("`martialArtsId` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("fed_rank")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("fed_rank")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("fed_rank")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->martialArts->DbValue = $row['martialArts'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("FedMartialartsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "FedMartialartsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "FedMartialartsAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "FedMartialartsView";
            case Config("API_ADD_ACTION"):
                return "FedMartialartsAdd";
            case Config("API_EDIT_ACTION"):
                return "FedMartialartsEdit";
            case Config("API_DELETE_ACTION"):
                return "FedMartialartsDelete";
            case Config("API_LIST_ACTION"):
                return "FedMartialartsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("FedMartialartsView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("FedMartialartsView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "FedMartialartsAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "FedMartialartsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("FedMartialartsEdit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("FedMartialartsEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("FedMartialartsAdd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("FedMartialartsAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("FedMartialartsDelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        // martialArts
        $this->martialArts->setupEditAttributes();
        $this->martialArts->EditCustomAttributes = "";
        if (!$this->martialArts->Raw) {
            $this->martialArts->CurrentValue = HtmlDecode($this->martialArts->CurrentValue);
        }
        $this->martialArts->EditValue = $this->martialArts->CurrentValue;
        $this->martialArts->PlaceHolder = RemoveHtml($this->martialArts->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->martialArts);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->martialArts);
