 ...
 * Page class
 */
class FinOrderView extends FinOrder
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FinOrderView";
    ...
    public function run()
    {
        ...
        $this->masterSchool->setVisibility();
        ...
        if ($this->isPageRequest()) { // Validate request
            ...
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "FinOrderList"; // Return to list
            }
                ...
                case "show": // Get a record to display
                    if (!$this->IsModal) { // Normal view page
                        ...
                        if ($this->TotalRecords <= 0) { // No record found
                            ...
                            $this->terminate("FinOrderList"); // Return to list page
                            return;
                        ...
                        if (!$matchRecord) {
                            ...
                            $returnUrl = "FinOrderList"; // No matching record, return to list
                        } else {
            ...
            $returnUrl = "FinOrderList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item = &$option->add("detail_fin_orderdetails");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("fin_orderdetails", "TblCaption");
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("FinOrderdetailsList?" . Config("TABLE_SHOW_MASTER") . "=fin_order&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("FinOrderdetailsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fin_order')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "fin_orderdetails";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fin_order')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "fin_orderdetails";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'fin_order')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    */
    public function exportData($return = false)
    {
        ...
        // Export detail records (fin_orderdetails)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("fin_orderdetails", explode(",", $this->getCurrentDetailTable()))) {
            $fin_orderdetails = new FinOrderdetailsList();
            $rsdetail = $fin_orderdetails->loadRs($fin_orderdetails->getDetailFilterFromSession()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->Table;
                    $doc->Table = $fin_orderdetails;
                    $fin_orderdetails->exportDocument($doc, new Recordset($rsdetail), 1, $detailcnt);
                    $doc->Table = $oldtbl;
                }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("fin_orderdetails", $detailTblVar)) {
                $detailPageObj = Container("FinOrderdetailsGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->orderId->CurrentValue = $this->id->CurrentValue;
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("FinOrderList"), "", $this->TableVar, true);
        $pageId = "view";
