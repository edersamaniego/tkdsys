...
/**
 * Table class for fin_debit
 */
class FinDebit extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'fin_debit';
        $this->TableName = 'fin_debit';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`fin_debit`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->accountId->Lookup = new Lookup('accountId', 'fin_accountspayable', false, 'id', ["id","value","",""], [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`id`, ''),'" . ValueSeparator(1, $this->accountId) . "',COALESCE(`value`,''))");
                break;
            case "pt-BR":
                $this->accountId->Lookup = new Lookup('accountId', 'fin_accountspayable', false, 'id', ["id","value","",""], [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`id`, ''),'" . ValueSeparator(1, $this->accountId) . "',COALESCE(`value`,''))");
                break;
        ...
        $this->dueDate->Required = true; // Required field
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->lastUser->Lookup = new Lookup('lastUser', 'school_users', false, 'id', ["name","lastName","",""], [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`name`, ''),'" . ValueSeparator(1, $this->lastUser) . "',COALESCE(`lastName`,''))");
                break;
            case "pt-BR":
                $this->lastUser->Lookup = new Lookup('lastUser', 'school_users', false, 'id', ["name","lastName","",""], [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`name`, ''),'" . ValueSeparator(1, $this->lastUser) . "',COALESCE(`lastName`,''))");
                break;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`fin_debit`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("FinDebitList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "FinDebitEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "FinDebitAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("FinDebitView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("FinDebitView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "FinDebitAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "FinDebitAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("FinDebitEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("FinDebitAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("FinDebitDelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "fin_accountspayable" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM `fin_debit`";
    ...
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "fin_accountspayable") {
            $filterWrk = Container("fin_accountspayable")->addUserIDFilter($filterWrk);
        }
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        $isIncome = ExecuteScalar("SELECT incomeReceivable FROM fin_accountspayable WHERE id = ".$rsnew['accountId']." ");
        if(isset($isIncome) && $isIncome > 0){
        	$this->CancelMessage = "You can not add any payment to this item";
        	return false;
        }
        return true;
    }
    ...
    public function rowInserted($rsold, &$rsnew)
    {
        //Log("Row Inserted");
         if(isset($rsnew["accountId"])){
            	$accountValue = ExecuteScalar("SELECT value FROM fin_accountspayable WHERE id = ".$rsnew['accountId']."");
            	updateAccountStatus($rsnew["accountId"], $accountValue, 2);
           }
    }
    ...
    public function rowUpdating($rsold, &$rsnew)
    {
        // Enter your code here
        // To cancel, set return value to false
        $isIncome = ExecuteScalar("SELECT incomeReceivable FROM fin_accountspayable WHERE id = ".$rsold['accountId']." ");
        if(isset($isIncome) && $isIncome > 0){
        	$this->CancelMessage = 'You can not update any payment to this item';
        	return false;
        }
        return true;
    }
    ...
    public function rowUpdated($rsold, &$rsnew)
    {
        //Log("Row Updated");
         if(isset($rsold["accountId"])){
           	$accountValue = ExecuteScalar("SELECT value FROM fin_accountspayable WHERE id = ".$rsold['accountId']."");
            updateAccountStatus($rsold["accountId"], $accountValue, 2);
        }
    }
    ...
    public function rowDeleting(&$rs)
    {
        // Enter your code here
        // To cancel, set return value to False
        $isIncome = ExecuteScalar("SELECT incomeReceivable FROM fin_accountspayable WHERE id = ".$rs['accountId']." ");
        if(isset($isIncome) && $isIncome > 0){
        	$this->CancelMessage = 'You can not delete any payment to this item';
        	return false;
        }
        return true;
    }
    ...
    public function rowDeleted(&$rs)
    {
        //Log("Row Deleted");
         if(isset($rs["accountId"])){
            	$accountValue = ExecuteScalar("SELECT value FROM fin_accountspayable WHERE id = ".$rs['accountId']."");
            	updateAccountStatus($rs["accountId"], $accountValue, 2);
         }
    }
