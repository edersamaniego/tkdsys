 ...
 * Page class
 */
class FinCheckingaccountAdd extends FinCheckingaccount
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FinCheckingaccountAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("FinCheckingaccountList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "FinCheckingaccountList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "FinCheckingaccountView") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'bank' first before field var 'x_bank'
        $val = $CurrentForm->hasValue("bank") ? $CurrentForm->getValue("bank") : $CurrentForm->getValue("x_bank");
        if (!$this->bank->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->bank->Visible = false; // Disable update for API request
            } else {
                $this->bank->setFormValue($val);
            }
        }
        ...
        if (!$this->organizationId->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->organizationId->Visible = false; // Disable update for API request
            } else {
                $this->organizationId->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->organizationId->CurrentValue = $this->organizationId->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->bank->EditValue = HtmlEncode($this->bank->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->bank->Required) {
            if (!$this->bank->IsDetailKey && EmptyValue($this->bank->FormValue)) {
                $this->bank->addErrorMessage(str_replace("%s", $this->bank->caption(), $this->bank->RequiredErrorMessage));
            }
        }
        if ($this->responsable->Required) {
        ...
        if (in_array("fin_debit", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // bank
        $this->bank->setDbValueDef($rsnew, $this->bank->CurrentValue, null, false);
        ...
        // schoolId
        $this->schoolId->CurrentValue = CurrentUserSchoolID();
        ...
        // organizationId
        $this->organizationId->CurrentValue = CurrentOrganizationID();
        $this->organizationId->setDbValueDef($rsnew, $this->organizationId->CurrentValue, null);
