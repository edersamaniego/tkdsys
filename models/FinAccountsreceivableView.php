 ...
 * Page class
 */
class FinAccountsreceivableView extends FinAccountsreceivable
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FinAccountsreceivableView";
    ...
    public function run()
    {
        ...
        $this->deferred->setVisibility();
        $this->amountInstallments->setVisibility();
        $this->totalValueDeferred->setVisibility();
        $this->firstdateInstallment->setVisibility();
        $this->actualInstallment->setVisibility();
        ...
        if ($this->isPageRequest()) { // Validate request
            ...
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "FinAccountsreceivableList"; // Return to list
            }
                ...
                case "show": // Get a record to display
                    if (!$this->IsModal) { // Normal view page
                        ...
                        if (!$matchRecord) {
                            ...
                            $returnUrl = "FinAccountsreceivableList"; // No matching record, return to list
                        } else {
            ...
            $returnUrl = "FinAccountsreceivableList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("FinCreditList?" . Config("TABLE_SHOW_MASTER") . "=fin_accountsreceivable&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("FinCreditGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fin_accountsreceivable')) {
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fin_accountsreceivable')) {
        ...
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'fin_accountsreceivable')) {
