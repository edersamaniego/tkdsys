 ...
 * Page class
 */
class SchoolModalityList extends SchoolModality
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'school_modality';

    // Page object name
    public $PageObjName = "SchoolModalityList";
    ...
    public $FormName = "fschool_modalitylist";
    ...
    public function __construct()
    {
        ...
        // Table object (school_modality)
        if (!isset($GLOBALS["school_modality"]) || get_class($GLOBALS["school_modality"]) == PROJECT_NAMESPACE . "school_modality") {
            $GLOBALS["school_modality"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "SchoolModalityAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "SchoolModalityDelete";
        $this->MultiUpdateUrl = "SchoolModalityUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'school_modality');
        }
    ...
    public function run()
    {
        ...
        $this->nameBR->setVisibility();
        $this->nameES->setVisibility();
        $this->nameEN->setVisibility();
        ...
        $this->setupLookupOptions($this->nameES);
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fschool_modalitysrch");
        }
        ...
        $filterList = Concat($filterList, $this->nameBR->AdvancedSearch->toJson(), ","); // Field nameBR
        $filterList = Concat($filterList, $this->nameES->AdvancedSearch->toJson(), ","); // Field nameES
        $filterList = Concat($filterList, $this->nameEN->AdvancedSearch->toJson(), ","); // Field nameEN
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fschool_modalitysrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field nameBR
        $this->nameBR->AdvancedSearch->SearchValue = @$filter["x_nameBR"];
        $this->nameBR->AdvancedSearch->SearchOperator = @$filter["z_nameBR"];
        $this->nameBR->AdvancedSearch->SearchCondition = @$filter["v_nameBR"];
        $this->nameBR->AdvancedSearch->SearchValue2 = @$filter["y_nameBR"];
        $this->nameBR->AdvancedSearch->SearchOperator2 = @$filter["w_nameBR"];
        $this->nameBR->AdvancedSearch->save();

        // Field nameES
        $this->nameES->AdvancedSearch->SearchValue = @$filter["x_nameES"];
        $this->nameES->AdvancedSearch->SearchOperator = @$filter["z_nameES"];
        $this->nameES->AdvancedSearch->SearchCondition = @$filter["v_nameES"];
        $this->nameES->AdvancedSearch->SearchValue2 = @$filter["y_nameES"];
        $this->nameES->AdvancedSearch->SearchOperator2 = @$filter["w_nameES"];
        $this->nameES->AdvancedSearch->save();

        // Field nameEN
        $this->nameEN->AdvancedSearch->SearchValue = @$filter["x_nameEN"];
        $this->nameEN->AdvancedSearch->SearchOperator = @$filter["z_nameEN"];
        $this->nameEN->AdvancedSearch->SearchCondition = @$filter["v_nameEN"];
        $this->nameEN->AdvancedSearch->SearchValue2 = @$filter["y_nameEN"];
        $this->nameEN->AdvancedSearch->SearchOperator2 = @$filter["w_nameEN"];
        $this->nameEN->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->nameBR;
        $searchFlds[] = &$this->nameES;
        $searchFlds[] = &$this->nameEN;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->nameBR); // nameBR
            $this->updateSort($this->nameES); // nameES
            $this->updateSort($this->nameEN); // nameEN
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fschool_modalitylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fschool_modalitylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("nameBR", $this->createColumnOption("nameBR"));
            $option->add("nameES", $this->createColumnOption("nameES"));
            $option->add("nameEN", $this->createColumnOption("nameEN"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fschool_modalitysrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fschool_modalitysrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fschool_modalitylist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->nameBR->setDbValue($row['nameBR']);
        $this->nameES->setDbValue($row['nameES']);
        $this->nameEN->setDbValue($row['nameEN']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['nameBR'] = $this->nameBR->DefaultValue;
        $row['nameES'] = $this->nameES->DefaultValue;
        $row['nameEN'] = $this->nameEN->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // nameBR
            $this->nameBR->ViewValue = $this->nameBR->CurrentValue;
            $this->nameBR->ViewCustomAttributes = "";

            // nameES
            $curVal = strval($this->nameES->CurrentValue);
            if ($curVal != "") {
                $this->nameES->ViewValue = $this->nameES->lookupCacheOption($curVal);
                if ($this->nameES->ViewValue === null) { // Lookup from database
                    $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                    $sqlWrk = $this->nameES->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->nameES->Lookup->renderViewRow($rswrk[0]);
                        $this->nameES->ViewValue = $this->nameES->displayValue($arwrk);
                    } else {
                        $this->nameES->ViewValue = $this->nameES->CurrentValue;
                    }
                }
            } else {
                $this->nameES->ViewValue = null;
            }
            $this->nameES->ViewCustomAttributes = "";

            // nameEN
            $this->nameEN->ViewValue = $this->nameEN->CurrentValue;
            $this->nameEN->ViewCustomAttributes = "";
            ...
            // nameBR
            $this->nameBR->LinkCustomAttributes = "";
            $this->nameBR->HrefValue = "";
            $this->nameBR->TooltipValue = "";

            // nameES
            $this->nameES->LinkCustomAttributes = "";
            $this->nameES->HrefValue = "";
            $this->nameES->TooltipValue = "";

            // nameEN
            $this->nameEN->LinkCustomAttributes = "";
            $this->nameEN->HrefValue = "";
            $this->nameEN->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fschool_modalitylist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fschool_modalitysrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
