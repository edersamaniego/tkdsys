 ...
 * Page class
 */
class FedVideoList extends FedVideo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedVideoList";
    ...
    public $FormName = "ffed_videolist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "FedVideoDelete";
        $this->MultiUpdateUrl = "FedVideoUpdate";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffed_videosrch");
        }
        ...
        $filterList = Concat($filterList, $this->frame->AdvancedSearch->toJson(), ","); // Field frame
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffed_videosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field frame
        $this->frame->AdvancedSearch->SearchValue = @$filter["x_frame"];
        $this->frame->AdvancedSearch->SearchOperator = @$filter["z_frame"];
        $this->frame->AdvancedSearch->SearchCondition = @$filter["v_frame"];
        $this->frame->AdvancedSearch->SearchValue2 = @$filter["y_frame"];
        $this->frame->AdvancedSearch->SearchOperator2 = @$filter["w_frame"];
        $this->frame->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->subsection, $default, false); // subsection
        ...
        $this->buildSearchSql($where, $this->frame, $default, false); // frame
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->subsection->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->frame->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_videolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_videolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"ffed_videolist\"><label class=\"form-check-label\" for=\"key\">" . $Language->phrase("SelectAll") . "</label></div></div>";
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffed_videosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffed_videosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffed_videolist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // section
        if ($this->section->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->section->AdvancedSearch->SearchValue != "" || $this->section->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // subsection
        if ($this->subsection->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->subsection->AdvancedSearch->SearchValue != "" || $this->subsection->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // updateDate
        if ($this->updateDate->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->updateDate->AdvancedSearch->SearchValue != "" || $this->updateDate->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // frame
        if ($this->frame->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->frame->AdvancedSearch->SearchValue != "" || $this->frame->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = trim(strval($this->section->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->section->AdvancedSearch->ViewValue = $this->section->lookupCacheOption($curVal);
            } else {
                $this->section->AdvancedSearch->ViewValue = $this->section->Lookup !== null && is_array($this->section->lookupOptions()) ? $curVal : null;
            }
            if ($this->section->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->section->EditValue = array_values($this->section->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->section->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
            ...
            $curVal = trim(strval($this->subsection->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->subsection->AdvancedSearch->ViewValue = $this->subsection->lookupCacheOption($curVal);
            } else {
                $this->subsection->AdvancedSearch->ViewValue = $this->subsection->Lookup !== null && is_array($this->subsection->lookupOptions()) ? $curVal : null;
            }
            if ($this->subsection->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->subsection->EditValue = array_values($this->subsection->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->subsection->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffed_videosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
