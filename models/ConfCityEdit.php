 ...
 * Page class
 */
class ConfCityEdit extends ConfCity
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ConfCityEdit";
    ...
    public function run()
    {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ConfCityList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ConfCityList") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->ufId->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->ufId->setFormValue($val, true, $validate);
            }
        }
        ...
        if (!$this->county->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->county->Visible = false; // Disable update for API request
            } else {
                $this->county->setFormValue($val);
            }
        }
