 ...
 * Page class
 */
class FedMartialartsView extends FedMartialarts
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedMartialartsView";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "FedMartialartsView") {
                        $row["view"] = "1";
                    }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            ...
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "FedMartialartsList"; // Return to list
            }
                ...
                case "show": // Get a record to display
                    if (!$this->IsModal) { // Normal view page
                        ...
                        if ($this->TotalRecords <= 0) { // No record found
                            ...
                            $this->terminate("FedMartialartsList"); // Return to list page
                            return;
                        ...
                        if (!$matchRecord) {
                            ...
                            $returnUrl = "FedMartialartsList"; // No matching record, return to list
                        } else {
            ...
            $returnUrl = "FedMartialartsList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        // "detail_fed_rank"
        $item = &$option->add("detail_fed_rank");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("fed_rank", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("FedRankList?" . Config("TABLE_SHOW_MASTER") . "=fed_martialarts&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'fed_martialarts')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'fed_martialarts')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'fed_martialarts')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fed_rank"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    */
    public function exportData($return = false)
    {
        ...
        // Export detail records (fed_rank)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("fed_rank", explode(",", $this->getCurrentDetailTable()))) {
            $fed_rank = new FedRankList();
            $rsdetail = $fed_rank->loadRs($fed_rank->getDetailFilterFromSession()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->Table;
                    $doc->Table = $fed_rank;
                    $fed_rank->exportDocument($doc, new Recordset($rsdetail), 1, $detailcnt);
                    $doc->Table = $oldtbl;
                }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("fed_rank", $detailTblVar)) {
                $detailPageObj = Container("FedRankGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->martialArtsId->IsDetailKey = true;
                    $detailPageObj->martialArtsId->CurrentValue = $this->id->CurrentValue;
                    $detailPageObj->martialArtsId->setSessionValue($detailPageObj->martialArtsId->CurrentValue);
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("FedMartialartsList"), "", $this->TableVar, true);
        $pageId = "view";
