 ...
 * Page class
 */
class FedVideosubsectionAdd extends FedVideosubsection
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FedVideosubsectionAdd";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "FedVideosubsectionView") {
                        $row["view"] = "1";
                    }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("FedVideosubsectionList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "FedVideosubsectionList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "FedVideosubsectionView") {
    ...
    public function renderRow()
    {
        ...
        // subsectionBr
        $this->subsectionBr->RowCssClass = "row";

        // subsectionSp
        $this->subsectionSp->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("fed_video", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("FedVideoGrid");
            if (in_array("fed_video", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->subsection->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "fed_video"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->subsection->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("fed_video", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailAdd) {
                    ...
                    $detailPageObj->subsection->IsDetailKey = true;
                    $detailPageObj->subsection->CurrentValue = $this->id->CurrentValue;
                    $detailPageObj->subsection->setSessionValue($detailPageObj->subsection->CurrentValue);
                    $detailPageObj->section->setSessionValue(""); // Clear session key
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("FedVideosubsectionList"), "", $this->TableVar, true);
