 ...
 * Page class
 */
class TesResultamountList extends TesResultamount
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TesResultamountList";
    ...
    public $FormName = "ftes_resultamountlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "TesResultamountAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "TesResultamountDelete";
        $this->MultiUpdateUrl = "TesResultamountUpdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->sendingDate); // sendingDate
            $this->updateSort($this->paymentDate); // paymentDate
            $this->updateSort($this->printingDate); // printingDate
            ...
            $this->updateSort($this->paymentId); // paymentId
            $this->updateSort($this->totalValue); // totalValue
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->testId->setSort("");
                $this->sendingDate->setSort("");
                $this->paymentDate->setSort("");
                $this->printingDate->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ftes_resultamountlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ftes_resultamountlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("sendingDate", $this->createColumnOption("sendingDate"));
            $option->add("paymentDate", $this->createColumnOption("paymentDate"));
            $option->add("printingDate", $this->createColumnOption("printingDate"));
            ...
            $option->add("totalAmount", $this->createColumnOption("totalAmount"));
            $option->add("paymentId", $this->createColumnOption("paymentId"));
            $option->add("totalValue", $this->createColumnOption("totalValue"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ftes_resultamountsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ftes_resultamountsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ftes_resultamountlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ftes_resultamountlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    */
    ...
    public function listOptionsLoad()
    {
        ...
        $opt = &$this->ListOptions->Add("printcertificate");
        $opt->Header = "Print";
        $opt->OnLeft = true; // Link on left
        $opt->MoveTo(0); // Move to first column
    }
    ...
    public function listOptionsRendered()
    {
        ...
        $this->ListOptions["printcertificate"]->Body = "certificate.php";
    }
