 ...
 * Page class
 */
class FinOrderAdd extends FinOrder
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FinOrderAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("FinOrderList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "FinOrderList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "FinOrderView") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'debtor' first before field var 'x_debtor'
        $val = $CurrentForm->hasValue("debtor") ? $CurrentForm->getValue("debtor") : $CurrentForm->getValue("x_debtor");
        if (!$this->debtor->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->debtor->Visible = false; // Disable update for API request
            } else {
                $this->debtor->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->due->CurrentValue = UnFormatDateTime($this->due->CurrentValue, $this->due->formatPattern());
        $this->debtor->CurrentValue = $this->debtor->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->debtor->EditValue = HtmlEncode($this->debtor->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->debtor->Required) {
            if (!$this->debtor->IsDetailKey && EmptyValue($this->debtor->FormValue)) {
                $this->debtor->addErrorMessage(str_replace("%s", $this->debtor->caption(), $this->debtor->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->debtor->FormValue)) {
            $this->debtor->addErrorMessage($this->debtor->getErrorMessage(false));
        }

        // Validate detail grid
        $detailTblVar = explode(",", $this->getCurrentDetailTable());
        $detailPage = Container("FinOrderdetailsGrid");
        if (in_array("fin_orderdetails", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // discountId
        $this->discountId->setDbValueDef($rsnew, $this->discountId->CurrentValue, null, false);
        ...
        // debtor
        $this->debtor->setDbValueDef($rsnew, $this->debtor->CurrentValue, null, false);
        ...
        if ($addRow) {
            ...
            $detailPage = Container("FinOrderdetailsGrid");
            if (in_array("fin_orderdetails", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->orderId->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "fin_orderdetails"); // Load user level of detail table
