 ...
 * Page class
 */
class FinOrderList extends FinOrder
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'fin_order';

    // Page object name
    public $PageObjName = "FinOrderList";
    ...
    public $FormName = "ffin_orderlist";
    ...
    public function __construct()
    {
        ...
        // Table object (fin_order)
        if (!isset($GLOBALS["fin_order"]) || get_class($GLOBALS["fin_order"]) == PROJECT_NAMESPACE . "fin_order") {
            $GLOBALS["fin_order"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "FinOrderAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FinOrderDelete";
        $this->MultiUpdateUrl = "FinOrderUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'fin_order');
        }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->discountId); // discountId
            $this->updateSort($this->date); // date
            $this->updateSort($this->due); // due
            $this->updateSort($this->debtor); // debtor
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_fin_orderdetails"
        $item = &$this->ListOptions->add("detail_fin_orderdetails");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'fin_orderdetails');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffin_orderlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffin_orderlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_fin_orderdetails"
        $opt = $this->ListOptions["detail_fin_orderdetails"];
        if ($Security->allowList(CurrentProjectID() . 'fin_orderdetails') && $this->showOptionLink()) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("fin_orderdetails", "TblCaption");
            if (!$this->ShowMultipleDetails) { // Skip loading record count if show multiple details
                $detailTbl = Container("fin_orderdetails");
                $detailFilter = $detailTbl->getDetailFilter($this);
                $detailTbl->setCurrentMasterTable($this->TableVar);
                $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
                $detailTbl->Count = $detailTbl->loadRecordCount($detailFilter);
                $body .= "&nbsp;" . str_replace("%c", Container("fin_orderdetails")->Count, $Language->phrase("DetailCount"));
            }
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("FinOrderdetailsList?" . Config("TABLE_SHOW_MASTER") . "=fin_order&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("FinOrderdetailsGrid");
            if ($detailPage->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'fin_order')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "fin_orderdetails";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'fin_order')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails");
                ...
                $detailEditTblVar .= "fin_orderdetails";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $this->showOptionLink("add") && $Security->allowAdd(CurrentProjectID() . 'fin_order')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails");
                ...
                $detailCopyTblVar .= "fin_orderdetails";
            }
            if ($links != "") {
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Preview extension
        $links = "";
        $sqlwrk = "`orderId`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";

        // Column "detail_fin_orderdetails"
        if ($this->DetailPages && $this->DetailPages["fin_orderdetails"] && $this->DetailPages["fin_orderdetails"]->Visible && $Security->allowList(CurrentProjectID() . 'fin_orderdetails')) {
            $link = "";
            $option = $this->ListOptions["detail_fin_orderdetails"];
            $url = "FinOrderdetailsPreview?t=fin_order&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"fin_orderdetails\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'fin_order')) {
                $label = $Language->TablePhrase("fin_orderdetails", "TblCaption");
                ...
                $label .= "&nbsp;" . JsEncode(str_replace("%c", Container("fin_orderdetails")->Count, $Language->phrase("DetailCount")));
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"fin_orderdetails\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("FinOrderdetailsList?" . Config("TABLE_SHOW_MASTER") . "=fin_order&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $title = $Language->TablePhrase("fin_orderdetails", "TblCaption");
            ...
            $detailPageObj = Container("FinOrderdetailsGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'fin_order')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'fin_order')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailAdd && $Security->canAdd() && $this->showOptionLink("add") && $Security->allowAdd(CurrentProjectID() . 'fin_order')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            $btngrp .= "</div>";
            if ($link != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_fin_orderdetails");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=fin_orderdetails");
                $detailPage = Container("FinOrderdetailsGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'fin_order') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "fin_orderdetails";
                }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("discountId", $this->createColumnOption("discountId"));
            $option->add("date", $this->createColumnOption("date"));
            $option->add("due", $this->createColumnOption("due"));
            $option->add("debtor", $this->createColumnOption("debtor"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffin_ordersrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffin_ordersrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffin_orderlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        ...
        $row['debtor'] = $this->debtor->DefaultValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffin_orderlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
