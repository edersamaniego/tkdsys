 ...
 * Page class
 */
class SchoolClassList extends SchoolClass
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SchoolClassList";
    ...
    public $FormName = "fschool_classlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "SchoolClassAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "SchoolClassDelete";
        $this->MultiUpdateUrl = "SchoolClassUpdate";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fschool_classsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->schoolId->AdvancedSearch->toJson(), ","); // Field schoolId
        $filterList = Concat($filterList, $this->daysByWeek->AdvancedSearch->toJson(), ","); // Field daysByWeek
        $filterList = Concat($filterList, $this->beginnigTime->AdvancedSearch->toJson(), ","); // Field beginnigTime
        $filterList = Concat($filterList, $this->endingTime->AdvancedSearch->toJson(), ","); // Field endingTime
        $filterList = Concat($filterList, $this->status->AdvancedSearch->toJson(), ","); // Field status
        $filterList = Concat($filterList, $this->limit->AdvancedSearch->toJson(), ","); // Field limit
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fschool_classsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field daysByWeek
        $this->daysByWeek->AdvancedSearch->SearchValue = @$filter["x_daysByWeek"];
        $this->daysByWeek->AdvancedSearch->SearchOperator = @$filter["z_daysByWeek"];
        $this->daysByWeek->AdvancedSearch->SearchCondition = @$filter["v_daysByWeek"];
        $this->daysByWeek->AdvancedSearch->SearchValue2 = @$filter["y_daysByWeek"];
        $this->daysByWeek->AdvancedSearch->SearchOperator2 = @$filter["w_daysByWeek"];
        $this->daysByWeek->AdvancedSearch->save();

        // Field beginnigTime
        $this->beginnigTime->AdvancedSearch->SearchValue = @$filter["x_beginnigTime"];
        $this->beginnigTime->AdvancedSearch->SearchOperator = @$filter["z_beginnigTime"];
        $this->beginnigTime->AdvancedSearch->SearchCondition = @$filter["v_beginnigTime"];
        $this->beginnigTime->AdvancedSearch->SearchValue2 = @$filter["y_beginnigTime"];
        $this->beginnigTime->AdvancedSearch->SearchOperator2 = @$filter["w_beginnigTime"];
        $this->beginnigTime->AdvancedSearch->save();

        // Field endingTime
        $this->endingTime->AdvancedSearch->SearchValue = @$filter["x_endingTime"];
        $this->endingTime->AdvancedSearch->SearchOperator = @$filter["z_endingTime"];
        $this->endingTime->AdvancedSearch->SearchCondition = @$filter["v_endingTime"];
        $this->endingTime->AdvancedSearch->SearchValue2 = @$filter["y_endingTime"];
        $this->endingTime->AdvancedSearch->SearchOperator2 = @$filter["w_endingTime"];
        $this->endingTime->AdvancedSearch->save();
        ...
        // Field limit
        $this->limit->AdvancedSearch->SearchValue = @$filter["x_limit"];
        $this->limit->AdvancedSearch->SearchOperator = @$filter["z_limit"];
        $this->limit->AdvancedSearch->SearchCondition = @$filter["v_limit"];
        $this->limit->AdvancedSearch->SearchValue2 = @$filter["y_limit"];
        $this->limit->AdvancedSearch->SearchOperator2 = @$filter["w_limit"];
        $this->limit->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->daysByWeek); // daysByWeek
            $this->updateSort($this->beginnigTime); // beginnigTime
            $this->updateSort($this->endingTime); // endingTime
            $this->updateSort($this->status); // status
            $this->updateSort($this->limit); // limit
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->daysByWeek->setSort("");
                $this->beginnigTime->setSort("");
                $this->endingTime->setSort("");
                $this->status->setSort("");
                $this->limit->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fschool_classlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fschool_classlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("daysByWeek", $this->createColumnOption("daysByWeek"));
            $option->add("beginnigTime", $this->createColumnOption("beginnigTime"));
            $option->add("endingTime", $this->createColumnOption("endingTime"));
            $option->add("status", $this->createColumnOption("status"));
            $option->add("limit", $this->createColumnOption("limit"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fschool_classsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fschool_classsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fschool_classlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fschool_classlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fschool_classsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
