 ...
 * Page class
 */
class FedFilescategoryList extends FedFilescategory
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'fed_filescategory';

    // Page object name
    public $PageObjName = "FedFilescategoryList";
    ...
    public $FormName = "ffed_filescategorylist";
    ...
    public function __construct()
    {
        ...
        // Table object (fed_filescategory)
        if (!isset($GLOBALS["fed_filescategory"]) || get_class($GLOBALS["fed_filescategory"]) == PROJECT_NAMESPACE . "fed_filescategory") {
            $GLOBALS["fed_filescategory"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "FedFilescategoryAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FedFilescategoryDelete";
        $this->MultiUpdateUrl = "FedFilescategoryUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'fed_filescategory');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("fed_filescategory");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->categoryBr->setVisibility();
        $this->categorySp->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "ffed_filescategorysrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->category->AdvancedSearch->toJson(), ","); // Field category
        $filterList = Concat($filterList, $this->categoryBr->AdvancedSearch->toJson(), ","); // Field categoryBr
        $filterList = Concat($filterList, $this->categorySp->AdvancedSearch->toJson(), ","); // Field categorySp
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffed_filescategorysrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field categoryBr
        $this->categoryBr->AdvancedSearch->SearchValue = @$filter["x_categoryBr"];
        $this->categoryBr->AdvancedSearch->SearchOperator = @$filter["z_categoryBr"];
        $this->categoryBr->AdvancedSearch->SearchCondition = @$filter["v_categoryBr"];
        $this->categoryBr->AdvancedSearch->SearchValue2 = @$filter["y_categoryBr"];
        $this->categoryBr->AdvancedSearch->SearchOperator2 = @$filter["w_categoryBr"];
        $this->categoryBr->AdvancedSearch->save();

        // Field categorySp
        $this->categorySp->AdvancedSearch->SearchValue = @$filter["x_categorySp"];
        $this->categorySp->AdvancedSearch->SearchOperator = @$filter["z_categorySp"];
        $this->categorySp->AdvancedSearch->SearchCondition = @$filter["v_categorySp"];
        $this->categorySp->AdvancedSearch->SearchValue2 = @$filter["y_categorySp"];
        $this->categorySp->AdvancedSearch->SearchOperator2 = @$filter["w_categorySp"];
        $this->categorySp->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->categoryBr;
        $searchFlds[] = &$this->categorySp;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->categoryBr); // categoryBr
            $this->updateSort($this->categorySp); // categorySp
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->categoryBr->setSort("");
                $this->categorySp->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_filescategorylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffed_filescategorylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("categoryBr", $this->createColumnOption("categoryBr"));
            $option->add("categorySp", $this->createColumnOption("categorySp"));
            $option->add("userId", $this->createColumnOption("userId"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffed_filescategorysrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffed_filescategorysrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffed_filescategorylist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->categoryBr->setDbValue($row['categoryBr']);
        $this->categorySp->setDbValue($row['categorySp']);
    ...
    protected function newRow()
    {
        ...
        $row['categoryBr'] = $this->categoryBr->DefaultValue;
        $row['categorySp'] = $this->categorySp->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // categoryBr
            $this->categoryBr->ViewValue = $this->categoryBr->CurrentValue;
            $this->categoryBr->ViewCustomAttributes = "";

            // categorySp
            $this->categorySp->ViewValue = $this->categorySp->CurrentValue;
            $this->categorySp->ViewCustomAttributes = "";
            ...
            // categoryBr
            $this->categoryBr->LinkCustomAttributes = "";
            $this->categoryBr->HrefValue = "";
            $this->categoryBr->TooltipValue = "";

            // categorySp
            $this->categorySp->LinkCustomAttributes = "";
            $this->categorySp->HrefValue = "";
            $this->categorySp->TooltipValue = "";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="ffed_filescategorylist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffed_filescategorysrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
