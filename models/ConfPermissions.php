...
/**
 * Table class for conf_permissions
 */
class ConfPermissions extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'conf_permissions';
        $this->TableName = 'conf_permissions';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`conf_permissions`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->_table->IsPrimaryKey = true; // Primary key field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`conf_permissions`";
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            ...
            if (array_key_exists('table', $rs)) {
                AddFilter($where, QuotedName('table', $this->Dbid) . '=' . QuotedValue($rs['table'], $this->_table->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        return "`userlevelid` = @userlevelid@ AND `table` = '@_table@'";
    }
    ...
    public function getKey($current = false)
    {
        ...
        $val = $current ? $this->_table->CurrentValue : $this->_table->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 2) {
            ...
            if ($current) {
                $this->_table->CurrentValue = $keys[1];
            } else {
                $this->_table->OldValue = $keys[1];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('table', $row) ? $row['table'] : null;
        } else {
            $val = $this->_table->OldValue !== null ? $this->_table->OldValue : $this->_table->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@_table@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ConfPermissionsList");
    }
    ...
    {
        global $Language;
        if ($pageName == "ConfPermissionsView") {
            return $Language->phrase("View");
        } elseif ($pageName == "ConfPermissionsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ConfPermissionsAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "ConfPermissionsView";
            case Config("API_ADD_ACTION"):
                return "ConfPermissionsAdd";
            case Config("API_EDIT_ACTION"):
                return "ConfPermissionsEdit";
            case Config("API_DELETE_ACTION"):
                return "ConfPermissionsDelete";
            case Config("API_LIST_ACTION"):
                return "ConfPermissionsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ConfPermissionsView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ConfPermissionsView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "ConfPermissionsAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "ConfPermissionsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ConfPermissionsEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ConfPermissionsAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("ConfPermissionsDelete", $this->getUrlParm());
    }
    ...
    public function keyToJson($htmlEncode = false)
    {
        ...
        $json .= ",\"_table\":" . JsonEncode($this->_table->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->_table->CurrentValue !== null) {
            $url .= $this->RouteCompositeKeySeparator . $this->encodeKeyValue($this->_table->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("_table") ?? Route("_table")) !== null) {
                $arKey[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->_table->CurrentValue = $key[1];
            } else {
                $this->_table->OldValue = $key[1];
            }
