...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fconf_ufedit = new ew.Form("fconf_ufedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fconf_ufedit;

    // Add fields
    var fields = currentTable.fields;
    fconf_ufedit.addFields([
    ...
    // Form_CustomValidate
    fconf_ufedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fconf_ufedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fconf_ufedit");
});
</script>
<script>
...
<?php } ?>
<form name="fconf_ufedit" id="fconf_ufedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="conf_uf" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
