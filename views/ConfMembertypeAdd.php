...
// Page object
$ConfMembertypeAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { conf_membertype: currentTable } });
var currentForm, currentPageID;
var fconf_membertypeadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fconf_membertypeadd = new ew.Form("fconf_membertypeadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fconf_membertypeadd;

    // Add fields
    var fields = currentTable.fields;
    fconf_membertypeadd.addFields([
    ...
    // Form_CustomValidate
    fconf_membertypeadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fconf_membertypeadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fconf_membertypeadd.lists.isStudent = <?= $Page->isStudent->toClientList($Page) ?>;
    fconf_membertypeadd.lists.isOwner = <?= $Page->isOwner->toClientList($Page) ?>;
    loadjs.done("fconf_membertypeadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fconf_membertypeadd" id="fconf_membertypeadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="form-check d-inline-block">
    <input type="checkbox" class="form-check-input<?= $Page->isStudent->isInvalidClass() ?>" data-table="conf_membertype" data-field="x_isStudent" name="x_isStudent[]" id="x_isStudent_334388" value="1"<?= ConvertToBool($Page->isStudent->CurrentValue) ? " checked" : "" ?><?= $Page->isStudent->editAttributes() ?> aria-describedby="x_isStudent_help">
...
<div class="form-check d-inline-block">
    <input type="checkbox" class="form-check-input<?= $Page->isOwner->isInvalidClass() ?>" data-table="conf_membertype" data-field="x_isOwner" name="x_isOwner[]" id="x_isOwner_173236" value="1"<?= ConvertToBool($Page->isOwner->CurrentValue) ? " checked" : "" ?><?= $Page->isOwner->editAttributes() ?> aria-describedby="x_isOwner_help">
