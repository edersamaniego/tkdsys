...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffed_filesedit = new ew.Form("ffed_filesedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = ffed_filesedit;

    // Add fields
    var fields = currentTable.fields;
    ffed_filesedit.addFields([
    ...
    // Form_CustomValidate
    ffed_filesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffed_filesedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    ffed_filesedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    ffed_filesedit.lists.category = <?= $Page->category->toClientList($Page) ?>;
    ffed_filesedit.lists.type = <?= $Page->type->toClientList($Page) ?>;
    loadjs.done("ffed_filesedit");
});
</script>
<script>
...
<?php } ?>
<form name="ffed_filesedit" id="ffed_filesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" name="fa_x_thumbs" id= "fa_x_thumbs" value="<?= (Post("fa_x_thumbs") == "0") ? "0" : "1" ?>">
...
<span id="el_fed_files_category">
    <select
        ...
        data-select2-id="ffed_filesedit_x_category[]"
        data-table="fed_files"
...
<script>
loadjs.ready("ffed_filesedit", function() {
    var options = { name: "x_category[]", selectId: "ffed_filesedit_x_category[]" },
    ...
    if (ffed_filesedit.lists.category.lookupOptions.length) {
        options.data = { id: "x_category[]", form: "ffed_filesedit" };
    } else {
        options.ajax = { id: "x_category[]", form: "ffed_filesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_fed_files_type">
    <select
        ...
        data-select2-id="ffed_filesedit_x_type[]"
        data-table="fed_files"
        data-field="x_type"
        data-dropdown
        multiple
...
<script>
loadjs.ready("ffed_filesedit", function() {
    var options = { name: "x_type[]", selectId: "ffed_filesedit_x_type[]" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.multiple = true;
    options.closeOnSelect = false;
    options.columns = el.dataset.repeatcolumn || 5;
    ...
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffed_filesedit.lists.type.lookupOptions.length) {
        options.data = { id: "x_type[]", form: "ffed_filesedit" };
    } else {
        options.ajax = { id: "x_type[]", form: "ffed_filesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="fed_files" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
