...
// Page object
$FedMartialartsEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { fed_martialarts: currentTable } });
var currentForm, currentPageID;
var ffed_martialartsedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffed_martialartsedit = new ew.Form("ffed_martialartsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = ffed_martialartsedit;

    // Add fields
    var fields = currentTable.fields;
    ffed_martialartsedit.addFields([
    ...
    // Form_CustomValidate
    ffed_martialartsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffed_martialartsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("ffed_martialartsedit");
});
</script>
<script>
...
<?php } ?>
<form name="ffed_martialartsedit" id="ffed_martialartsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["ffed_martialartsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffed_martialartsedit", "x_createDate", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="fed_martialarts" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php
    if (in_array("fed_rank", explode(",", $Page->getCurrentDetailTable())) && $fed_rank->DetailEdit) {
?>
