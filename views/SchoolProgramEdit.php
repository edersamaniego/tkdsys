...
// Page object
$SchoolProgramEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { school_program: currentTable } });
var currentForm, currentPageID;
var fschool_programedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fschool_programedit = new ew.Form("fschool_programedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fschool_programedit;

    // Add fields
    var fields = currentTable.fields;
    fschool_programedit.addFields([
    ...
    // Form_CustomValidate
    fschool_programedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fschool_programedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fschool_programedit.lists.modalityId = <?= $Page->modalityId->toClientList($Page) ?>;
    fschool_programedit.lists.schoolId = <?= $Page->schoolId->toClientList($Page) ?>;
    loadjs.done("fschool_programedit");
});
</script>
<script>
...
<?php } ?>
<form name="fschool_programedit" id="fschool_programedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_school_program_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_school_program_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="school_program" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready("fschool_programedit", function() {
    fschool_programedit.createAutoSuggest(Object.assign({"id":"x_modalityId","forceSelect":false}, ew.vars.tables.school_program.fields.modalityId.autoSuggestOptions));
});
</script>
...
<span id="el_school_program_schoolId">
    <select
        ...
        data-select2-id="fschool_programedit_x_schoolId"
...
<script>
loadjs.ready("fschool_programedit", function() {
    var options = { name: "x_schoolId", selectId: "fschool_programedit_x_schoolId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fschool_programedit.lists.schoolId.lookupOptions.length) {
        options.data = { id: "x_schoolId", form: "fschool_programedit" };
    } else {
        options.ajax = { id: "x_schoolId", form: "fschool_programedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("fschool_programedit", function() {
    fschool_programedit.createAutoSuggest(Object.assign({"id":"x_schoolId","forceSelect":false}, ew.vars.tables.school_program.fields.schoolId.autoSuggestOptions));
});
</script>
<?= $Page->schoolId->Lookup->getParamTag($Page, "p_x_schoolId") ?>
</span>
