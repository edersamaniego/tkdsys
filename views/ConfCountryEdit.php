...
// Page object
$ConfCountryEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { conf_country: currentTable } });
var currentForm, currentPageID;
var fconf_countryedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fconf_countryedit = new ew.Form("fconf_countryedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fconf_countryedit;

    // Add fields
    var fields = currentTable.fields;
    fconf_countryedit.addFields([
    ...
    // Form_CustomValidate
    fconf_countryedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fconf_countryedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fconf_countryedit");
});
</script>
<script>
...
<?php } ?>
<form name="fconf_countryedit" id="fconf_countryedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="conf_country" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
