...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffed_schooledit = new ew.Form("ffed_schooledit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = ffed_schooledit;

    // Add fields
    var fields = currentTable.fields;
    ffed_schooledit.addFields([
    ...
    // Form_CustomValidate
    ffed_schooledit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffed_schooledit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    ffed_schooledit.lists.countryId = <?= $Page->countryId->toClientList($Page) ?>;
    ffed_schooledit.lists.UFId = <?= $Page->UFId->toClientList($Page) ?>;
    ffed_schooledit.lists.cityId = <?= $Page->cityId->toClientList($Page) ?>;
    ffed_schooledit.lists.isheadquarter = <?= $Page->isheadquarter->toClientList($Page) ?>;
    loadjs.done("ffed_schooledit");
});
</script>
<script>
...
<?php } ?>
<form name="ffed_schooledit" id="ffed_schooledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php $Page->countryId->EditAttrs->prepend("onchange", "ew.updateOptions.call(this);"); ?>
    <select
        ...
        data-select2-id="ffed_schooledit_x_countryId"
...
<script>
loadjs.ready("ffed_schooledit", function() {
    var options = { name: "x_countryId", selectId: "ffed_schooledit_x_countryId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffed_schooledit.lists.countryId.lookupOptions.length) {
        options.data = { id: "x_countryId", form: "ffed_schooledit" };
    } else {
        options.ajax = { id: "x_countryId", form: "ffed_schooledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="ffed_schooledit_x_UFId"
        data-table="fed_school"
        data-field="x_UFId"
...
<script>
loadjs.ready("ffed_schooledit", function() {
    var options = { name: "x_UFId", selectId: "ffed_schooledit_x_UFId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffed_schooledit.lists.UFId.lookupOptions.length) {
        options.data = { id: "x_UFId", form: "ffed_schooledit" };
    } else {
        options.ajax = { id: "x_UFId", form: "ffed_schooledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="ffed_schooledit_x_cityId"
        data-table="fed_school"
        data-field="x_cityId"
...
<script>
loadjs.ready("ffed_schooledit", function() {
    var options = { name: "x_cityId", selectId: "ffed_schooledit_x_cityId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffed_schooledit.lists.cityId.lookupOptions.length) {
        options.data = { id: "x_cityId", form: "ffed_schooledit" };
    } else {
        options.ajax = { id: "x_cityId", form: "ffed_schooledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["ffed_schooledit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffed_schooledit", "x_openingDate", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["ffed_schooledit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffed_schooledit", "x_birthDateOwner", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<div class="form-check d-inline-block">
    <input type="checkbox" class="form-check-input<?= $Page->isheadquarter->isInvalidClass() ?>" data-table="fed_school" data-field="x_isheadquarter" name="x_isheadquarter[]" id="x_isheadquarter_941914" value="1"<?= ConvertToBool($Page->isheadquarter->CurrentValue) ? " checked" : "" ?><?= $Page->isheadquarter->editAttributes() ?> aria-describedby="x_isheadquarter_help">
...
<?php } ?>
    ...
    <input type="hidden" data-table="fed_school" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
...
<?php
    if (in_array("school_users", explode(",", $Page->getCurrentDetailTable())) && $school_users->DetailEdit) {
?>
...
?>
<?php
    if (in_array("school_member", explode(",", $Page->getCurrentDetailTable())) && $school_member->DetailEdit) {
?>
