...
// Page object
$FinAccountspayableEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { fin_accountspayable: currentTable } });
var currentForm, currentPageID;
var ffin_accountspayableedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffin_accountspayableedit = new ew.Form("ffin_accountspayableedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = ffin_accountspayableedit;

    // Add fields
    var fields = currentTable.fields;
    ffin_accountspayableedit.addFields([
        ...
        ["accountFather", [fields.accountFather.visible && fields.accountFather.required ? ew.Validators.required(fields.accountFather.caption) : null, ew.Validators.integer], fields.accountFather.isInvalid],
    ...
    // Form_CustomValidate
    ffin_accountspayableedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffin_accountspayableedit.validateRequired = ew.CLIENT_VALIDATE;

    // Multi-Page
    ffin_accountspayableedit.multiPage = new ew.MultiPage("ffin_accountspayableedit");

    // Dynamic selection lists
    ffin_accountspayableedit.lists.departamentId = <?= $Page->departamentId->toClientList($Page) ?>;
    ffin_accountspayableedit.lists.costCenterId = <?= $Page->costCenterId->toClientList($Page) ?>;
    ffin_accountspayableedit.lists.employeeId = <?= $Page->employeeId->toClientList($Page) ?>;
    ffin_accountspayableedit.lists.creditorsId = <?= $Page->creditorsId->toClientList($Page) ?>;
    ffin_accountspayableedit.lists.typeId = <?= $Page->typeId->toClientList($Page) ?>;
    ffin_accountspayableedit.lists.lastUserId = <?= $Page->lastUserId->toClientList($Page) ?>;
    loadjs.done("ffin_accountspayableedit");
});
</script>
<script>
...
<?php } ?>
<form name="ffin_accountspayableedit" id="ffin_accountspayableedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="ew-nav<?= $Page->MultiPages->containerClasses() ?>" id="pages_FinAccountspayableEdit"><!-- multi-page tabs -->
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_fin_accountspayable_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_fin_accountspayable_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="fin_accountspayable" data-field="x_id" data-hidden="1" data-page="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_fin_accountspayable_departamentId">
    <select
        ...
        data-select2-id="ffin_accountspayableedit_x_departamentId"
...
<script>
loadjs.ready("ffin_accountspayableedit", function() {
    var options = { name: "x_departamentId", selectId: "ffin_accountspayableedit_x_departamentId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffin_accountspayableedit.lists.departamentId.lookupOptions.length) {
        options.data = { id: "x_departamentId", form: "ffin_accountspayableedit" };
    } else {
        options.ajax = { id: "x_departamentId", form: "ffin_accountspayableedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_fin_accountspayable_costCenterId">
    <select
        ...
        data-select2-id="ffin_accountspayableedit_x_costCenterId"
        data-table="fin_accountspayable"
...
<script>
loadjs.ready("ffin_accountspayableedit", function() {
    var options = { name: "x_costCenterId", selectId: "ffin_accountspayableedit_x_costCenterId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffin_accountspayableedit.lists.costCenterId.lookupOptions.length) {
        options.data = { id: "x_costCenterId", form: "ffin_accountspayableedit" };
    } else {
        options.ajax = { id: "x_costCenterId", form: "ffin_accountspayableedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["ffin_accountspayableedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffin_accountspayableedit", "x_issue", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["ffin_accountspayableedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffin_accountspayableedit", "x_due", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_fin_accountspayable_employeeId">
    <select
        ...
        data-select2-id="ffin_accountspayableedit_x_employeeId"
        data-table="fin_accountspayable"
...
<script>
loadjs.ready("ffin_accountspayableedit", function() {
    var options = { name: "x_employeeId", selectId: "ffin_accountspayableedit_x_employeeId" };
    if (ffin_accountspayableedit.lists.employeeId.lookupOptions.length) {
        options.data = { id: "x_employeeId", form: "ffin_accountspayableedit" };
    } else {
        options.ajax = { id: "x_employeeId", form: "ffin_accountspayableedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="ffin_accountspayableedit_x_creditorsId"
        data-table="fin_accountspayable"
...
<script>
loadjs.ready("ffin_accountspayableedit", function() {
    var options = { name: "x_creditorsId", selectId: "ffin_accountspayableedit_x_creditorsId" };
    if (ffin_accountspayableedit.lists.creditorsId.lookupOptions.length) {
        options.data = { id: "x_creditorsId", form: "ffin_accountspayableedit" };
    } else {
        options.ajax = { id: "x_creditorsId", form: "ffin_accountspayableedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="ffin_accountspayableedit_x_typeId"
        data-table="fin_accountspayable"
        data-field="x_typeId"
        data-page="1"
...
<script>
loadjs.ready("ffin_accountspayableedit", function() {
    var options = { name: "x_typeId", selectId: "ffin_accountspayableedit_x_typeId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffin_accountspayableedit.lists.typeId.lookupOptions.length) {
        options.data = { id: "x_typeId", form: "ffin_accountspayableedit" };
    } else {
        options.ajax = { id: "x_typeId", form: "ffin_accountspayableedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php if ($Page->accountFather->Visible) { // accountFather ?>
    <div id="r_accountFather"<?= $Page->accountFather->rowAttributes() ?>>
        <label id="elh_fin_accountspayable_accountFather" for="x_accountFather" class="<?= $Page->LeftColumnClass ?>"><?= $Page->accountFather->caption() ?><?= $Page->accountFather->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->accountFather->cellAttributes() ?>>
<span id="el_fin_accountspayable_accountFather">
<input type="<?= $Page->accountFather->getInputTextType() ?>" name="x_accountFather" id="x_accountFather" data-table="fin_accountspayable" data-field="x_accountFather" value="<?= $Page->accountFather->EditValue ?>" data-page="1" size="30" placeholder="<?= HtmlEncode($Page->accountFather->getPlaceHolder()) ?>"<?= $Page->accountFather->editAttributes() ?> aria-describedby="x_accountFather_help">
<?= $Page->accountFather->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->accountFather->getErrorMessage() ?></div>
</span>
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" name="fa_x_invoiceFile" id= "fa_x_invoiceFile" value="<?= (Post("fa_x_invoiceFile") == "0") ? "0" : "1" ?>">
...
<input type="hidden" name="fa_x_guaranteeFile" id= "fa_x_guaranteeFile" value="<?= (Post("fa_x_guaranteeFile") == "0") ? "0" : "1" ?>">
...
<input type="hidden" name="fa_x_attachedFile" id= "fa_x_attachedFile" value="<?= (Post("fa_x_attachedFile") == "0") ? "0" : "1" ?>">
