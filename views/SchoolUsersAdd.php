...
var fschool_usersadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fschool_usersadd = new ew.Form("fschool_usersadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fschool_usersadd;

    // Add fields
    var fields = currentTable.fields;
    fschool_usersadd.addFields([
    ...
    // Form_CustomValidate
    fschool_usersadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fschool_usersadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fschool_usersadd.lists.schoolIdMaster = <?= $Page->schoolIdMaster->toClientList($Page) ?>;
    fschool_usersadd.lists.schoolId = <?= $Page->schoolId->toClientList($Page) ?>;
    fschool_usersadd.lists.createUserId = <?= $Page->createUserId->toClientList($Page) ?>;
    fschool_usersadd.lists.level = <?= $Page->level->toClientList($Page) ?>;
    loadjs.done("fschool_usersadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fschool_usersadd" id="fschool_usersadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_school_users_schoolIdMaster">
    <select
        ...
        data-select2-id="fschool_usersadd_x_schoolIdMaster"
...
<script>
loadjs.ready("fschool_usersadd", function() {
    var options = { name: "x_schoolIdMaster", selectId: "fschool_usersadd_x_schoolIdMaster" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fschool_usersadd.lists.schoolIdMaster.lookupOptions.length) {
        options.data = { id: "x_schoolIdMaster", form: "fschool_usersadd" };
    } else {
        options.ajax = { id: "x_schoolIdMaster", form: "fschool_usersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("fschool_usersadd", function() {
    fschool_usersadd.createAutoSuggest(Object.assign({"id":"x_schoolIdMaster","forceSelect":false}, ew.vars.tables.school_users.fields.schoolIdMaster.autoSuggestOptions));
});
</script>
<?= $Page->schoolIdMaster->Lookup->getParamTag($Page, "p_x_schoolIdMaster") ?>
</span>
...
<span id="el_school_users_schoolId">
    <select
        ...
        data-select2-id="fschool_usersadd_x_schoolId"
        data-table="school_users"
...
<script>
loadjs.ready("fschool_usersadd", function() {
    var options = { name: "x_schoolId", selectId: "fschool_usersadd_x_schoolId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fschool_usersadd.lists.schoolId.lookupOptions.length) {
        options.data = { id: "x_schoolId", form: "fschool_usersadd" };
    } else {
        options.ajax = { id: "x_schoolId", form: "fschool_usersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_school_users_level">
    <select
        ...
        data-select2-id="fschool_usersadd_x_level"
        data-table="school_users"
        data-field="x_level"
...
<script>
loadjs.ready("fschool_usersadd", function() {
    var options = { name: "x_level", selectId: "fschool_usersadd_x_level" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fschool_usersadd.lists.level.lookupOptions.length) {
        options.data = { id: "x_level", form: "fschool_usersadd" };
    } else {
        options.ajax = { id: "x_level", form: "fschool_usersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
