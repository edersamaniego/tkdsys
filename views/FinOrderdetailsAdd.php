...
// Page object
$FinOrderdetailsAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { fin_orderdetails: currentTable } });
var currentForm, currentPageID;
var ffin_orderdetailsadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffin_orderdetailsadd = new ew.Form("ffin_orderdetailsadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = ffin_orderdetailsadd;

    // Add fields
    var fields = currentTable.fields;
    ffin_orderdetailsadd.addFields([
    ...
    // Form_CustomValidate
    ffin_orderdetailsadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffin_orderdetailsadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    ffin_orderdetailsadd.lists.discountId = <?= $Page->discountId->toClientList($Page) ?>;
    loadjs.done("ffin_orderdetailsadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="ffin_orderdetailsadd" id="ffin_orderdetailsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready("ffin_orderdetailsadd", function() {
    ffin_orderdetailsadd.createAutoSuggest(Object.assign({"id":"x_discountId","forceSelect":false}, ew.vars.tables.fin_orderdetails.fields.discountId.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["ffin_orderdetailsadd", "editor"], function() {
    ew.createEditor("ffin_orderdetailsadd", "x_observation", 35, 4, <?= $Page->observation->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
