...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffin_debitedit = new ew.Form("ffin_debitedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = ffin_debitedit;

    // Add fields
    var fields = currentTable.fields;
    ffin_debitedit.addFields([
    ...
    // Form_CustomValidate
    ffin_debitedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffin_debitedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    ffin_debitedit.lists.accountId = <?= $Page->accountId->toClientList($Page) ?>;
    ffin_debitedit.lists.paymentMethod = <?= $Page->paymentMethod->toClientList($Page) ?>;
    ffin_debitedit.lists.checkingAccountId = <?= $Page->checkingAccountId->toClientList($Page) ?>;
    ffin_debitedit.lists.lastUser = <?= $Page->lastUser->toClientList($Page) ?>;
    loadjs.done("ffin_debitedit");
});
</script>
<script>
...
<?php } ?>
<form name="ffin_debitedit" id="ffin_debitedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_fin_debit_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_fin_debit_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="fin_debit" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready("ffin_debitedit", function() {
    ffin_debitedit.createAutoSuggest(Object.assign({"id":"x_accountId","forceSelect":false}, ew.vars.tables.fin_debit.fields.accountId.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["ffin_debitedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffin_debitedit", "x_dueDate", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_fin_debit_paymentMethod">
    <select
        ...
        data-select2-id="ffin_debitedit_x_paymentMethod"
...
<script>
loadjs.ready("ffin_debitedit", function() {
    var options = { name: "x_paymentMethod", selectId: "ffin_debitedit_x_paymentMethod" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffin_debitedit.lists.paymentMethod.lookupOptions.length) {
        options.data = { id: "x_paymentMethod", form: "ffin_debitedit" };
    } else {
        options.ajax = { id: "x_paymentMethod", form: "ffin_debitedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_fin_debit_checkingAccountId">
    <select
        ...
        data-select2-id="ffin_debitedit_x_checkingAccountId"
        data-table="fin_debit"
...
<script>
loadjs.ready("ffin_debitedit", function() {
    var options = { name: "x_checkingAccountId", selectId: "ffin_debitedit_x_checkingAccountId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffin_debitedit.lists.checkingAccountId.lookupOptions.length) {
        options.data = { id: "x_checkingAccountId", form: "ffin_debitedit" };
    } else {
        options.ajax = { id: "x_checkingAccountId", form: "ffin_debitedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["ffin_debitedit", "editor"], function() {
    ew.createEditor("ffin_debitedit", "x_obs", 35, 4, <?= $Page->obs->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
