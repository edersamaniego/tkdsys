...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ftes_aprovedadd = new ew.Form("ftes_aprovedadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = ftes_aprovedadd;

    // Add fields
    var fields = currentTable.fields;
    ftes_aprovedadd.addFields([
    ...
    // Form_CustomValidate
    ftes_aprovedadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ftes_aprovedadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("ftes_aprovedadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="ftes_aprovedadd" id="ftes_aprovedadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_tes_aproved_schoolId">
    <select
        ...
        data-select2-id="ftes_aprovedadd_x_schoolId"
...
<script>
loadjs.ready("ftes_aprovedadd", function() {
    var options = { name: "x_schoolId", selectId: "ftes_aprovedadd_x_schoolId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftes_aprovedadd.lists.schoolId.lookupOptions.length) {
        options.data = { id: "x_schoolId", form: "ftes_aprovedadd" };
    } else {
        options.ajax = { id: "x_schoolId", form: "ftes_aprovedadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["ftes_aprovedadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ftes_aprovedadd", "x_createDate", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
