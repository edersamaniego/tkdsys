...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fconf_cityedit = new ew.Form("fconf_cityedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fconf_cityedit;

    // Add fields
    var fields = currentTable.fields;
    fconf_cityedit.addFields([
    ...
    // Form_CustomValidate
    fconf_cityedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fconf_cityedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fconf_cityedit.lists.uf = <?= $Page->uf->toClientList($Page) ?>;
    fconf_cityedit.lists.ufId = <?= $Page->ufId->toClientList($Page) ?>;
    loadjs.done("fconf_cityedit");
});
</script>
<script>
...
<?php } ?>
<form name="fconf_cityedit" id="fconf_cityedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_conf_city_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_conf_city_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="conf_city" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready("fconf_cityedit", function() {
    fconf_cityedit.createAutoSuggest(Object.assign({"id":"x_uf","forceSelect":false}, ew.vars.tables.conf_city.fields.uf.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready("fconf_cityedit", function() {
    fconf_cityedit.createAutoSuggest(Object.assign({"id":"x_ufId","forceSelect":false}, ew.vars.tables.conf_city.fields.ufId.autoSuggestOptions));
});
</script>
