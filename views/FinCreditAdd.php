...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffin_creditadd = new ew.Form("ffin_creditadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = ffin_creditadd;

    // Add fields
    var fields = currentTable.fields;
    ffin_creditadd.addFields([
    ...
    // Form_CustomValidate
    ffin_creditadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffin_creditadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    ffin_creditadd.lists.accountId = <?= $Page->accountId->toClientList($Page) ?>;
    ffin_creditadd.lists.paymentMethod = <?= $Page->paymentMethod->toClientList($Page) ?>;
    ffin_creditadd.lists.checkingAccountId = <?= $Page->checkingAccountId->toClientList($Page) ?>;
    ffin_creditadd.lists.masterSchoolId = <?= $Page->masterSchoolId->toClientList($Page) ?>;
    loadjs.done("ffin_creditadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="ffin_creditadd" id="ffin_creditadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready("ffin_creditadd", function() {
    ffin_creditadd.createAutoSuggest(Object.assign({"id":"x_accountId","forceSelect":false}, ew.vars.tables.fin_credit.fields.accountId.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["ffin_creditadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffin_creditadd", "x_dueDate", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_fin_credit_paymentMethod">
    <select
        ...
        data-select2-id="ffin_creditadd_x_paymentMethod"
...
<script>
loadjs.ready("ffin_creditadd", function() {
    var options = { name: "x_paymentMethod", selectId: "ffin_creditadd_x_paymentMethod" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffin_creditadd.lists.paymentMethod.lookupOptions.length) {
        options.data = { id: "x_paymentMethod", form: "ffin_creditadd" };
    } else {
        options.ajax = { id: "x_paymentMethod", form: "ffin_creditadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_fin_credit_checkingAccountId">
    <select
        ...
        data-select2-id="ffin_creditadd_x_checkingAccountId"
        data-table="fin_credit"
...
<script>
loadjs.ready("ffin_creditadd", function() {
    var options = { name: "x_checkingAccountId", selectId: "ffin_creditadd_x_checkingAccountId" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffin_creditadd.lists.checkingAccountId.lookupOptions.length) {
        options.data = { id: "x_checkingAccountId", form: "ffin_creditadd" };
    } else {
        options.ajax = { id: "x_checkingAccountId", form: "ffin_creditadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
